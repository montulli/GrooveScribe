...
abc2svg.sym_name = ['bar', 'clef', 'custos', 'smark', 'grace', 'key', 'meter', 'Zrest', 'note', 'part', 'rest', 'yspace', 'staves', 'Break', 'tempo', '', 'block', 'remark']
...
abc2svg.Abc = function(user) {
    ...
    var decos = {
        ...
        f_near = [d_near, d_slide, d_arp],
        f_note = [null, null, null, d_upstaff, d_upstaff],
        f_staff = [null, null, null, null, null, d_upstaff, d_pf, d_upstaff]
    ...
    function d_cresc(de) {
        ...
        while (1) {
            ...
            if (de3 == de || de3 == de2)
                continue
            if (!(de.up ^ de3.up) && f_staff[de3.dd.func]) {
    ...
    function d_pf(de) {
        ...
        if (de.ldst)
            return
        if ((de.pos & C.SL_ALI_MSK) == C.SL_CLOSE || ((de.pos & C.SL_ALI_MSK) == 0 && s.fmt.dynalign < 0)) {
            d_upstaff(de)
            return
        }
        if (de.start) {
    ...
    function d_upstaff(de) {
        ...
        de.up = up = (dd.func == 4 || dd.func == 7) ? 0 : dd.func == 6 ? up6(s, de.pos) : up3(s, de.pos)
        switch (dd.ty) {
            case '@':
            case '<':
            case '>':
            ...
            switch (dd.func) {
                ...
                case 34:
                    if (s.type != C.NOTE || !prev || prev.type != C.NOTE || s.dur != prev.dur) {
                        error(1, s, "!$1! must be on the last of a couple of notes", nm)
                        continue
                    }
                    s.trem2 = true;
                    s.beam_end = true;
    ...
    Abc.prototype.draw_all_deco = function() {
        if (!a_de.length)
            return
        var de, dd, s, note, f, st, x, y, y2, ym, uf, i, str, a, new_de = [],
            ymid = []
        st = nstaff;
        y = staff_tb[st].y
        while (--st >= 0) {
        ...
        while (1) {
            ...
            } else if (dd.func == 6 && ((de.pos & C.SL_ALI_MSK) == C.SL_CENTER || ((de.pos & C.SL_ALI_MSK) == 0 && !s.fmt.dynalign)) && ((de.up && st > 0) || (!de.up && st < nstaff))) {
                if (de.up)
                    ym = ymid[--st]
                else
                    ym = ymid[st++];
    ...
    function draw_deco_near() {
        ...
        function create_deco(s) {
            var dd, k, pos, de, x, y, v, nd = s.a_dd.length
            if (s.y == undefined)
                s.y = 0
            for (k = 0; k < nd; k++) {
                dd = s.a_dd[k]
        ...
        function ll_deco() {
            var i, j, de, de2, de3, dd, dd2, v, s, st, n_de = a_de.length
            for (i = 0; i < n_de; i++) {
                de = a_de[i]
                if (!de.ldst)
                    continue
    ...
    function draw_deco_staff() {
        var s, p_voice, y, i, v, de, dd, minmax = new Array(nstaff + 1),
            nd = a_de.length

        function draw_repbra(p_voice) {
            var s, s1, x, y, y2, i, p, w, wh, first_repeat;
            y = staff_tb[p_voice.st].topbar + 15
            ...
            for (; s; s = s.next) {
                ...
                if (s1.text)
                    xy_str(x + 4, y2 - gene.curfont.size - 2, s1.text);
                xypath(x, y2);
                if (s1.rbstart == 2)
                    output += 'm0 10v-10';
        ...
        for (i = 0; i < nd; i++) {
            ...
            if (dd.dd_en)
                continue
            if ((de.pos & C.SL_ALI_MSK) == C.SL_ALIGN || ((de.pos & C.SL_ALI_MSK) == 0 && de.s.fmt.dynalign > 0)) {
                if (de.up) {
        ...
        for (i = 0; i < nd; i++) {
            ...
            if (dd.dd_en || !f_staff[dd.func])
                continue
            if (dd.func == 6 && ((de.pos & C.SL_ALI_MSK) == C.SL_ALIGN || ((de.pos & C.SL_ALI_MSK) == 0 && de.s.fmt.dynalign > 0))) {
                if (de.up)
                    y = minmax[de.st].ymax
                else
                    y = minmax[de.st].ymin;
    ...
    function draw_partempo(st, top) {
        var s, s2, some_part, some_tempo, h, w, y, dy = 0,
            ht = 0
    ...
    function draw_beams(bm) {
        var s, i, beam_dir, shift, bshift, bstub, bh, da, bd, k, k1, k2, x1, s1 = bm.s1,
            s2 = bm.s2
    ...
    function draw_lstaff(x) {
        var i, j, yb, h, fl, nst = cur_sy.nstaff,
            l = 0
...
<text x="X" y="Y" text-anchor="middle">A</text>\n', x, y + 12, m_gl(meter.top))
    ...
    Abc.prototype.draw_hl = function(s) {
        ...
        if (!n)
            return
        var dx1, dx2, hl, shhd, hlp, stafflines = p_staff.stafflines,
    ...
    function draw_rest(s) {
        var s2, i, j, y, bx, p_staff = staff_tb[s.st],
            yb = p_staff.y,
            x = s.x
        if (s.notes[0].shhd)
    ...
    function draw_mrest(s) {
        var x1, x2, prev, p_st = staff_tb[s.st],
    ...
    function grace_slur(s) {
        var yy, x0, y0, x3, y3, bet1, bet2, dy1, dy2, last, below, so = s,
            g = s.extra
        while (1) {
            if (!g.next)
                break
    ...
    function draw_note(s, fl) {
        var s2, i, m, y, slen, c, nflags, y_tb = new Array(s.nhd + 1),
            note = s.notes[s.stem < 0 ? s.nhd : 0],
            x = x_head(s, note),
            y = y_head(s, note),
            staffb = staff_tb[s.st].y
        ...
        function draw_sls(s, sl, snote) {
            ...
            switch (sl.ty & 0x07) {
                ...
                case C.SL_BELOW:
                    ...
                    break
                default:
                    dir = s.v != s2.v ? 1 : slur_direction(s, s2)
                    sl.ty &= ~0x07
    ...
    function draw_tuplet(s1) {
        var s2, s3, g, upstaff, nb_only, x1, x2, y1, y2, xm, ym, a, s0, yy, yx, dy, a, dir, r, tp = s1.tp.shift()
        if (!s1.tp.length)
            delete s1.tp
        upstaff = s1.st
        set_dscale(s1.st)
    ...
    function draw_tie(not1, not2, job) {
        var m, x1, s, y, h, time, p = job == 2 ? not1.pit : not2.pit,
    ...
    function draw_all_ties(p_voice) {
        var s, s1, s2, clef_chg, x, dx, m, not1, not2, tim2 = 0
        s1 = p_voice.sym
        set_color(s1.color)
        for (; s1; s1 = s1.next) {
            if (s1.ti2 && s1.time != tim2) {
                for (m = 0; m <= s1.nhd; m++) {
                    not2 = s1.notes[m]
                    not1 = not2.tie_s
    ...
    function draw_sym_near() {
        var p_voice, p_st, s, v, st, y, g, w, i, st, dx, top, bot, ymn, slur, output_sav = output;

        function set_yab(s1, s2) {
            var y, k = realwidth / YSTEP,
                i = (s1.x / k) | 0,
                j = (s2.x / k) | 0,
    ...
    function draw_vname(indent, stl) {
        var p_voice, n, st, v, a_p, p, y, h, h2, staff_d = []
    ...
    function set_staff() {
        var i, st, prev_staff, v, y, staffsep, dy, maxsep, mbot, val, p_voice, p_staff, fmt = tsfirst.fmt,
            sy = cur_sy
        for (v = 0; v < voice_tb.length; v++) {
            p_voice = voice_tb[v]
            if (p_voice.scale != 1)
        ...
        for (st = 0; st <= nstaff; st++) {
            ...
            if (p_staff.staffscale != 1) {
                p_staff.scale_str = 'transform="translate(0,' +
    ...
    function draw_systems(indent) {
        var s, s2, st, x, x2, res, sy, xstaff = [],
            stl = [],
            bar_bot = [],
            bar_height = [],
            ba = [],
                ...
                for (; i < l; i++, y -= il, dy -= il) {
                    switch (stafflines[i]) {
                        ...
                        case '-':
                            continue
                        case ty:
                            ln += 'm-' + w.toFixed(1) + ' ' + dy + 'h' + w.toFixed(1);
                            dy = 0
                            continue
                    }
                    if (ty != undefined)
                    ...
                    ty = stafflines[i]
                    ln += '<path class="' +
                        (ty == '[' ? 'slthW' : 'slW') + '" d="m0 ' + y + 'h' + w.toFixed(1);
                    dy = 0
                }
                ln += '"/>'
            }
        ...
        function draw_bar(s, bot, h) {
            var i, s2, yb, w, bar_type = s.bar_type,
                st = s.st,
                p_staff = staff_tb[st],
                x = s.x
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            switch (s.type) {
                case C.STAVES:
                    ...
                    continue
                case C.BAR:
                    if (s.invis || !s.bar_type || !cur_sy.st_print[s.st])
                        break
                    ...
                    if (cur_sy.voices[s.v] && cur_sy.voices[s.v].range == 0) {
                        if (s.xmx > 14) {
                            ...
                            for (var i = 0; i < voice_tb.length; i++) {
                                if (cur_sy.voices[i] && cur_sy.voices[i].range > 0)
                                    nv++
                            }
    ...
    function set_tie_room() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            for (; s; s = s.next) {
                if (!s.ti1)
                    continue
                if (s.notes[0].pit < 20 && s.notes[0].tie_ty && (s.notes[0].tie_ty & 0x07) == C.SL_BELOW);
                else if (s.notes[s.nhd].pit > 24 && s.notes[s.nhd].tie_ty && (s.notes[s.nhd].tie_ty & 0x07) == C.SL_ABOVE);
                else
                    continue
                s2 = s.next
...
") format("truetype")'
    ...
    var cfmt = {
        ...
        gracespace: new Float32Array([6, 8, 11]),
        graceslurs: true,
        headerfont: {
            name: txt_ff,
            size: 16
...
H "History: "',
        ...
        scale: 1,
        slurheight: 1.0,
        spatab: new Float32Array([10.2, 13.3, 17.3, 22.48, 29.2, 38, 49.4, 64.2, 83.5, 108.5]),
        staffsep: 46,
        stemheight: 21,
        stretchlast: .25,
        stretchstaff: true,
    ...
    Abc.prototype.set_format = function(cmd, param) {
        ...
        switch (cmd) {
            ...
            case "pos":
                ...
                if (cmd[1].slice(0, 3) == 'tup' && curvoice) {
                    if (!curvoice.tup)
 ...
 src:" + font.src + "}")
    ...
    function acc_shift(notes, dx_head) {
        var i, i1, i2, dx, dx1, dx2, ps, p1, acc, n = notes.length
        for (i = n - 1; --i >= 0;) {
            dx = notes[i].shhd
            if (!dx || dx > 0)
                continue
    ...
    function unlksym(s) {
        ...
        if (s.ts_next) {
            if (s.seqst) {
                ...
                if (s.ts_next.seqst && s.ts_prev && s.ts_prev.seqst && !w_tb[s.ts_prev.type]) {
    ...
    function set_graceoffs(s) {
        var next, m, dx, x, gspleft = s.fmt.gracespace[0],
    ...
    function set_w_chs(s) {
        ...
        for (; s; s = s.ts_next) {
            ...
            for (i = 0; i < s.a_gch.length; i++) {
                ...
                if (w0) {
                    if (w0 > x + ch.x) {
                        if (s.prev && s.prev.seqst && s.prev.type == C.BAR)
                            n--
                        dw = (w0 - x - ch.x) / n
                        while (1) {
                            s0 = s0.ts_next
    ...
    Abc.prototype.set_width = function(s) {
        var s2, i, m, xx, w, wlnote, wlw, acc, nt, bar_type, meter, last_acc, n1, n2, esp, tmp
        if (s.play) {
            s.wl = s.wr = 0
            return
        }
    ...
    function set_allsymwidth() {
        var val, st, s_chs, stup, itup, s = tsfirst,
            s2 = s,
            xa = 0,
            xl = [],
            wr = [],
    ...
    function set_nl(s) {
        ...
        function bardiv(so) {
            ...
            s = so
            while (s.ts_prev && s.ts_prev.time == so.time) {
                s = s.ts_prev
                if (s.bar_type) {
                    if (s.bar_type[0] == '[' && s.text && s.ts_next == so)
                        so = s
                    s1 = s
                }
            }
            ...
            while (s1 != so) {
                if (s1.bar_type && (s1.bar_type.slice(-1) == ':' || s1.text) && s1.bar_type != ':') {
                    new_type(s1)
                    s2 = clone(s1)
                    s1.bar_type = t1
                    delete s1.a_gch
            ...
            switch (s.type) {
                case C.STAVES:
                    ...
                    if (s.ts_prev.type == C.BAR)
                        break
                    while (s.ts_next) {
                        if (w_tb[s.ts_next.type] && s.ts_next.type != C.CLEF)
                            break
                        s = s.ts_next
                    }
    ...
    function set_lines(s, next, lwidth, indent) {
        var first, s2, s3, s4, s5, x, xmin, xmid, xmax, wwidth, shrink, space, nlines, last = next ? next.ts_prev : null,
            ws = get_width(s, next)
            ...
            if (!s3) {
                ...
                for (; s != next; s = s.ts_next) {
                    ...
                    if (beam || s.in_tuplet)
                        continue
                    if (s5) {
                        if (xmid - s5.x > x - xmid || (s.dur && (s.time + s.dur - bar_time) % (C.BLEN / 4) == 0))
                            s3 = s5
                    }
                    s5 = s
                }
    ...
    function mrest_expand() {
        ...
        function mexp(s) {
            var bar, s3, s4, tim, nbar, nb = s.nmes,
                dur = s.dur / nb,
                s2 = s.next
            while (!s2.bar_type)
                s2 = s2.next
    ...
    Abc.prototype.set_pitch = function(last_s) {
        var s, s2, g, st, delta, pitch, note, dur = C.BLEN,
            m = nstaff + 1,
            staff_delta = new Int16Array(new Array(m * 2)),
            sy = cur_sy
        for (st = 0; st <= nstaff; st++) {
            s = staff_tb[st].clef;
    ...
    function init_music_line() {
        var p_voice, s, s1, s2, s3, last_s, v, st, shr, shrmx, shl, fmt, shlp, p_st, top, nv = voice_tb.length
        for (v = 0; v < nv; v++) {
            if (!cur_sy.voices[v])
                continue
            p_voice = voice_tb[v];
    ...
    function set_words(p_voice) {
        var s, s2, nflags, lastnote, res, start_flag = true,
            pitch = 127
    ...
    function set_global() {
        var p_voice, v, nv = voice_tb.length,
            sy = cur_sy,
            st = sy.nstaff
    ...
    function get_lshift() {
        var st, v, p_v, p1, po, fnt, w, lsh1 = 0,
            lsho = 0,
            nv = voice_tb.length
    ...
    function set_indent(lsh) {
        var st, v, w, p_voice, p, i, j, font, vnt = 0,
            nv = voice_tb.length
        for (v = 0; v < nv; v++) {
            p_voice = voice_tb[v]
    ...
    function set_beams(sym) {
        var s, t, g, beam, s_opp, n, m, mid_p, pu, pd, laststem = -1
    ...
    function same_head(s1, s2) {
        var i1, i2, l1, l2, head, i11, i12, i21, i22, sh1, sh2, shu = s1.fmt.shiftunison || 0
        if (shu >= 3)
            return false
        if ((l1 = s1.dur) >= C.BLEN)
            return false
    ...
    function set_overlap() {
        var s, s1, s2, s3, i, i1, i2, m, sd, t, dp, d, d2, dr, dr2, dx, left1, right1, left2, right2, right3, pl, pr, sy = cur_sy
    ...
    Abc.prototype.set_sym_glue = function(width) {
        var s, g, ll, x, some_grace, spf, xmin = 0,
            xx = 0,
            xs = 0,
            xse = 0
        for (s = tsfirst; s; s = s.ts_next) {
    ...
    Abc.prototype.output_music = function() {
        var v, lwidth, indent, lsh, line_height, ts1st, tslast, p_v, nv = voice_tb.length
        set_global()
        if (nv > 1)
            self.set_stem_dir()
        for (v = 0; v < nv; v++)
    ...
    function new_key(param) {
        ...
        if (s.k_a_acc) {
            ...
            s.k_map = s.k_bagpipe && !sf ? abc2svg.keys[9] : abc2svg.keys[sf + 7]
        }
        s.k_mode = mode
    ...
    function new_tempo(text) {
        ...
        if (parse.state < 2 || (!curvoice.time && !glovar.tempo)) {
            info.Q = txt
            glovar.tempo = s
            return
        }
    ...
    function new_bar() {
        ...
        if (bar_type == "[" || bar_type == "|:") {
            ...
            if (s2 && s2.time == curvoice.time) {
                ...
                if (s2 && s2.type == C.BAR) {
                    if ((bar_type == "[" && !s2.text && (!curvoice.st || (par_sy.staves[curvoice.st - 1].flags & STOP_BAR) || s.norepbra)) || s2.bar_type == "|") {
                        if (bar_type != "[")
        ...
        sym_link(s);
        s.st = curvoice.st
        if (s.rbstart && bar_type != "[" && !curvoice.norepbra && s.st > 0 && !(par_sy.staves[s.st - 1].flags & STOP_BAR)) {
            s2 = {
                type: C.BAR,
                fname: s.fname,
                istart: s.istart,
    ...
    function do_ties(s, tie_s) {
        var i, m, not1, not2, mid, g, nt = 0,
            se = (tie_s.time + tie_s.dur) == curvoice.time
        for (m = 0; m <= s.nhd; m++) {
            not2 = s.notes[m]
            mid = not2.midi
    ...
    Abc.prototype.new_note = function(grace, sls) {
        var note, s, in_chord, c, dcn, type, tie_s, acc_tie, i, n, s2, nd, res, num, dur, apit, div, ty, sl1 = [],
            line = parse.line,
            a_dcn_sav = a_dcn
        a_dcn = []
    ...
    var sheet
    var add_fstyle = typeof document != "undefined" ? function(s) {
        var e
        if (cfmt.fullsvg)
            font_style += "\n" + s
        if (!sheet) {
    ...
    var
        sw_tb = new Float32Array([.000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .250, .333, .408, .500, .500, .833, .778, .333, .333, .333, .500, .564, .250, .564, .250, .278, .500, .500, .500, .500, .500, .500, .500, .500, .500, .500, .278, .278, .564, .564, .564, .444, .921, .722, .667, .667, .722, .611, .556, .722, .722, .333, .389, .722, .611, .889, .722, .722, .556, .722, .667, .556, .611, .722, .722, .944, .722, .722, .611, .333, .278, .333, .469, .500, .333, .444, .500, .444, .500, .444, .333, .500, .500, .278, .278, .500, .278, .778, .500, .500, .500, .500, .333, .389, .278, .500, .500, .722, .500, .500, .444, .480, .200, .480, .541, .500]),
        ssw_tb = new Float32Array([.000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .000, .278, .278, .355, .556, .556, .889, .667, .191, .333, .333, .389, .584, .278, .333, .278, .278, .556, .556, .556, .556, .556, .556, .556, .556, .556, .556, .278, .278, .584, .584, .584, .556, 1.015, .667, .667, .722, .722, .667, .611, .778, .722, .278, .500, .667, .556, .833, .722, .778, .667, .778, .722, .667, .611, .722, .667, .944, .667, .667, .611, .278, .278, .278, .469, .556, .333, .556, .556, .500, .556, .556, .278, .556, .556, .222, .222, .500, .222, .833, .556, .556, .556, .556, .333, .500, .278, .556, .500, .722, .500, .500, .500, .334, .260, .334, .584, .512]),
        cw_tb = sw_tb
    ...
    var strwh = typeof document != "undefined" ? (function() {
    ...
    function str2svg(str) {
        if (typeof str == "object")
            return str
        var n_font, wh, o_font = gene.deffont,
            c_font = gene.curfont,
            o = ""
    ...
    function xy_str(x, y, str, action, w, wh) {
        ...
        if (action != 'j' && str.length > 5 && gene.deffont.wadj)
            output += '" lengthAdjust="' + gene.deffont.wadj + '" textLength="' + wh[0].toFixed(1);
        output += '" x="';
        out_sxsy(x, '" y="', y + wh[1] * .2)
        switch (action) {
            case 'c':
                x -= wh[0] / 2;
    ...
    function write_text(text, action) {
        if (action == 's')
            return
        set_page();
        var wh, font, o, strlw = get_lwidth(),
    ...
    function put_words(words) {
        var p, i, j, nw, w, lw, x1, x2, i1, i2, do_flush, maxn = 0,
            n = 1
    ...
    function write_headform(lwidth) {
        var c, font, font_name, align, x, y, sz, w, info_val = {},
...
\n.box{outline:1px solid black;outline-offset:1px}',
    ...
    function set_scale(s) {
        var new_dy, new_scale = s.p_v.scale
        if (new_scale == 1) {
            set_sscale(s.st)
            return
        }
...
<tspan dy=".5" style="font-size:10px">mb</tspan></text>\n', x - 10, y);
 ...
 fill="currentColor" stroke-width=".7"',
        ...
        posy *= cfmt.scale
        if (user.imagesize) {
            head += user.imagesize + ' viewBox="0 0 ' + img.width.toFixed(0) + ' ' +
                posy.toFixed(0) + '">\n'
        } else {
    ...
    function sort_all() {
        var s, s2, time, w, wmin, ir, fmt, fl, new_sy, nv = voice_tb.length,
            vtb = [],
            vn = [],
            sy = cur_sy,
            v = sy.top_voice,
    ...
    function voice_adj(sys_chg) {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            for (; s; s = s.next) {
                if (((!w_tb[s.type] && s.type != C.STAVES) || s.type == C.SPACE || s.type == C.GRACE) && (!s.prev || s.time > s.prev.time + s.prev.dur)) {
                    s2 = {
                        type: C.SM,
                        v: s.v,
                        p_v: s.p_v,
    ...
    function new_syst(init) {
        var st, v, sy_staff, p_voice, sy_new = {
            voices: [],
            staves: [],
            top_voice: 0
        }
    ...
    Abc.prototype.set_bar_num = function() {
        var s, s2, tim, rep_tim, k, n, b_typ, bar_num = gene.nbar,
            bar_tim = 0,
            ptim = 0,
    ...
    function set_ctrl() {
        ...
        for (tim in parse.ctrl) {
            ...
            while (s && s.time < tim)
                s = s.ts_next
            if (!s) {} else {
                while (s.time == tim && s.ts_next && !s.dur)
                    s = s.ts_next
                if (e.part) {
                    s.part = e.part
                }
    ...
    function get_staves(cmd, parm) {
        var s, p_voice, p_voice2, i, flags, v, vid, a_vf, st, range, nv = voice_tb.length,
            maxtime = 0
        if (parm) {
    ...
    function get_key(parm) {
        ...
        if ((!s_key.k_a_acc || !s_key.k_a_acc.length) && !s_key.k_sf && !s_key.k_old_sf)
            s_key.invis = true
        if (!s_key.k_b40)
    ...
    function new_voice(id) {
        var v, p_v_sav, p_voice = voice_tb[0],
            n = voice_tb.length
    ...
    function get_voice(parm) {
        ...
        if (!curvoice.filtered && par_sy.voices[v] && (parse.voice_opts || parse.tune_v_opts)) {
        ...
        while (1) {
            ...
            switch (c) {
                ...
                case '"':
                    ...
                    i = p.indexOf(c, j)
                    if (i < 0) {
                        syntax(1, c == '!' ? "No end of decoration" : "No end of chord symbol/annotation");
                        i = p.length
                        continue
                    }
                    d = p.slice(j - 1, i + 1)
    ...
    function ly_set(s) {
        var i, j, ly, d, s1, s2, p, w, spw, xx, sz, shift, dw, s3 = s,
            wx = 0,
            wl = 0,
            n = 0,
            dx = 0,
    ...
    function draw_lyric_line(p_voice, j, y) {
        var p, lastx, w, s, s2, ly, lyl, ln, hyflag, lflag, x0, font, shift
    ...
    function draw_all_lyrics() {
        var p_voice, s, v, nly, i, x, y, w, a_ly, ly, lyst_tb = new Array(nstaff + 1),
            nv = voice_tb.length,
            h_tb = new Array(nv),
            nly_tb = new Array(nv),
    ...
    function parse_gchord(type) {
        var c, text, gch, x_abs, y_abs, i, j, istart, iend, ann_font = get_font("annotation"),
            h_ann = ann_font.size,
            line = parse.line
    ...
    function gch_tr1(p, transp) {
        var i, o, n, a, ip, b40, csa = p.split('/')
    ...
    function csan_add(s) {
        ...
        if (s.type == C.BAR) {
            for (i = 0; i < a_gch.length; i++) {
                if (a_gch[i].type == 'g') {
                    syntax(1, "There cannot be chord symbols on measure bars")
                    a_gch.splice(i)
                }
            }
        }
    ...
    Abc.prototype.set_a_gch = function(s, a) {
        a_gch = a;
        csan_add(s)
    }
    ...
    Abc.prototype.set_realwidth = function(v) {
        realwidth = v
    }
...
abc2svg.version = "1.22.1";
abc2svg.vdate = "2022-09-13"