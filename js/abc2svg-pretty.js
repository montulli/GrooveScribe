...
function Abc(user) {
    var wpsobj, svgobj;
    this.user = user;
    this.version = "1.3.2", this.vdate = "2015-09-04";
    const BAR = 0,
        CLEF = 1,
        CUSTOS = 2,
        FORMAT = 3,
    ...
    function error(sev, s, msg) {
        if (s) errbld(sev, msg, s.ctx.fname, s.istart);
        else errbld(sev, msg)
    }
    ...
    function scanBuf() {
        this.index = 0;
        this.char = function() {
            if (this.index >= this.buffer.length) return undefined;
        ...
        this.advance = function() {
            if (this.index < this.buffer.length) this.index++
        };
    ...
    function up_p(s, pos) {
        ...
        if (!voice_tb[s.v].have_ly) return false;
    ...
    function d_near(de) {
        ...
        if (up) y = Math.floor(s.ymx);
        else y = Math.floor(s.ymn - dd.h);
        if (y > -6 && y < 24) {
            if (up) y += 3;
    ...
    function d_pf(de) {
        ...
        if (!up) de.y -= dd.h;
        if (dd.str) set_str(de, dd.str);
        else if (dd.name != "sfz") de.str = dd.name
    }
    ...
    function d_trill(de) {
        ...
        if (de.start) {
            s = de.start.s;
            x = s.x;
            if (s.type == NOTE && s.a_dd && s.a_dd.length > 1) x += 10
        } else {
            s = first_note;
            x = s.x - s.wl - 4
        }
        ...
        if (up) {
            ...
            if (y < stafft) y = stafft
        } else {
            y -= dd.h;
            var staffb = staff_tb[s.st].botbar - 2;
            if (y > staffb) y = staffb
        }
    ...
    function deco_build(text) {
        ...
        if (c == "(" || c == ")" && dd.name.indexOf("(") < 0) {
            ...
            dd2 = dd_tb[name2];
            if (dd2) {
                if (c == "(") dd.ld_en_dd = dd2;
                else dd2.ld_en_dd = dd
            }
        }
        return dd
    }
    ...
    function deco_def(name) {
        ...
        if (user_deco_tb) {
            ...
            for (i = 0; i < n; i++) {
                if (user_deco_tb[i].indexOf(dn) == 0) return deco_build(user_deco_tb[i])
            }
        }
        n = std_deco_tb.length;
        for (i = 0; i < n; i++) {
            if (std_deco_tb[i].indexOf(dn) == 0) return deco_build(std_deco_tb[i])
        }
        return undefined
    }
    ...
    function deco_width(s) {
        ...
        if (wl != 0 && s.prev && s.prev.type == BAR) wl -= 3;
        return wl
    }
    ...
    function draw_all_deco() {
        ...
        while (1) {
            ...
            if (de.flags.grace) {
                ...
                out_sxsy(x, ",", y);
                if (de.flags.inv) output.push(') scale(0.7, -0.7)">\n');
    ...
    function create_deco(s) {
        ...
        for (k = 0; k < n; k++) {
            ...
            a_de.push(de);
            if (s.grace) de.flags.grace = true;
            if (dd.ldst) {
                de.flags.ldst = true
            } else if (dd.lden) {
                de.flags.lden = true;
    ...
    function draw_deco_staff() {
        ...
        if (first_gchord) {
            for (i = 0; i <= nstaff; i++) {
                ...
                minmax[i].ymin -= 3;
                if (minmax[i].ymin > bot - 10) minmax[i].ymin = bot - 10;
                var top = staff_tb[i].topbar;
                minmax[i].ymax += 3;
                if (minmax[i].ymax < top + 10) minmax[i].ymax = top + 10
            }
            set_sscale(-1);
        ...
        for (v = 0; v < n; v++) {
            ...
            for (s = p_voice.sym; s; s = s.next) {
                ...
                if (!s.next) break;
                if (!first_repeat) first_repeat = s;
                s1 = s;
                i = s1.text ? 4 : 2;
                for (;;) {
                    if (!s.next) break;
                    ...
                    if (s.bar_type.length > 1 && s.bar_type != "[]" || s.bar_type == "]" || s.text) break;
                    if (--i <= 0) {
                        s = s1;
                        i = 2;
                        for (;;) {
            ...
            for (; s; s = s.next) {
                ...
                if (s2.rbstop) {
                    ...
                    if (s.st > 0 && !(cur_sy.staves[s.st - 1].flags & STOP_BAR)) {
                        ...
                        w = 0;
                        if (s2.bar_type == "]") s2.invisible = true
                    } else {
                        w = 8
                    }
                } else {
                ...
                if (i == 0 && !s2.next && !s2.rbstop) {
                    if (!p_voice.bar_start) repnl = s1
                }
                if (i >= 0) {
                    xy_str(x + 4, y2 - gene.curfont.size * .9, p);
                    xypath(x, y2);
                    if (i != 1) output.push("m0 20v-20");
                    output.push("h");
                    output.push(w.toFixed(2));
                    if (i != 0) output.push("v20");
                    output.push('"/>\n');
        ...
        for (i = 0; i < n; i++) {
            de = a_de[i];
            dd = de.dd;
            if (dd.ldst || dd.func < 6) continue;
            if (cfmt.dynalign) {
                if (de.flags.up) y = minmax[de.st].ymax;
                else y = minmax[de.st].ymin;
                de.y = y
            } else {
                y = de.y
            }
            if (de.flags.up) y += dd.h;
    ...
    function draw_measnb() {
        ...
        for (; s; s = s.ts_next) {
            ...
            if (s.type != BAR || !s.bar_num) continue;
            bar_num = s.bar_num;
            if (cfmt.measurenb == 0 || bar_num % cfmt.measurenb != 0 || !s.next) continue;
            ...
            if (s.next.type == NOTE) {
                if (s.next.stem > 0) {
                    if (y < s.next.ys - gene.curfont.size) y = s.next.ys - gene.curfont.size
                } else {
    ...
    function tempo_width(s) {
        var i, n, w = 0;
        set_font("tempo");
        if (s.tempo_str1) w = strw(s.tempo_str1);
        if (s.tempo_value) {
            i = 1;
        ...
        if (s.tempo_str2) w += strw(s.tempo_str2);
        return w
    }
    ...
    function write_tempo(s, x, y, sc) {
        ...
        if (s.tempo_value) {
            ...
            n = s.tempo_notes.length;
            for (j = 0; j < n; j++) x += draw_notempo(s, x, y, s.tempo_notes[j], sc);
            x += 5;
            xy_str(x, y, "=");
            x += strw("= ") + 5;
            if (s.tempo_value.indexOf("/") < 0) {
                ...
                if (s.tempo_value >= 10) {
                    x += dx;
                    if (s.tempo_value >= 100) x += dx
                }
            } else {
                res = s.tempo_value.split("/");
                if (res.length == 2 && Number(res[1]) > 0) draw_notempo(s, x, y, res[0] * BASE_LEN / res[1], sc)
            }
        }
    ...
    function draw_partempo(st, top) {
        var s, g, some_part, some_tempo, h, w, y, dy = 0,
            ht = 0;
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            if (!g) continue;
            for (; g; g = g.next)
                if (g.type == TEMPO) break;
            if (!g) continue;
            if (!some_tempo) some_tempo = true;
            w = tempo_width(g);
            y = y_get(st, true, s.x - 5, w) + 2;
            if (y > ymin) ymin = y;
            if (x >= s.x - 5 && !(dosh & shift >> 1)) dosh |= shift;
            shift <<= 1;
            x = s.x - 5 + w
        }
        if (some_tempo) {
    ...
    function b_pos(grace, stem, nflags, b) {
        var top, bot, d1, d2, shift = !grace ? BEAM_SHIFT : 3,
    ...
    function calculate_beam(bm, s1) {
        ...
        for (s = s1;; s = s.next) {
            ...
            if ((scale = voice_tb[s.v].scale) == 1) scale = staff_tb[s.st].staffscale;
            if (s.stem >= 0) x = stem_xoff + s.notes[0].shhd;
            else x = -stem_xoff + s.notes[s.nhd].shhd;
            x *= scale;
            x += s.x;
            s.xs = x;
        ...
        if (!s1.grace) {
            if (notes >= 3) {
                hh = syy - a * sxy - b * sy;
                if (hh > 0 && hh / (notes - 2) > .5) a *= BEAM_FLATFAC
            }
            if (a >= 0) a = BEAM_SLOPE * a / (BEAM_SLOPE + a);
            else a = BEAM_SLOPE * a / (BEAM_SLOPE - a)
        } else {
            if (a > BEAM_SLOPE) a = BEAM_SLOPE;
            else if (a < -BEAM_SLOPE) a = -BEAM_SLOPE
        }
        a0 = a * (s2.xs - s1.xs) / (20 * (notes - 1));
        if (a0 * a0 < BEAM_THRESH * BEAM_THRESH) a = 0;
        b = (sy - a * sx) / notes;
        if (cfmt.flatbeams) {
            if (!s1.grace) b = -11 + staff_tb[st].y;
            else b = 35 + staff_tb[st].y;
            a = 0
        }
        max_stem_err = 0;
        s = s1;
        if (two_dir) {
            if (!s1.grace) ys = BEAM_SHIFT;
            else ys = 3;
            ys *= nflags - 1;
            ys += BEAM_DEPTH;
            ys *= .5;
            if (s1.stem != s2.stem && s1.nflags < s2.nflags) ys *= s2.stem;
            else ys *= s1.stem;
            b += ys
        } else if (!s1.grace) {
            var beam_h = BEAM_DEPTH + BEAM_SHIFT * (nflags - 1);
            while (s.ts_prev && s.ts_prev.type == NOTE && s.ts_prev.time == s.time && s.ts_prev.x > s1.xs) s = s.ts_prev;
            for (; s && s.time <= s2.time; s = s.ts_next) {
                ...
                if (s.v == v) {
                    if (s.nhd == 0) stem_err = min_tb[0][s.nflags];
                    else stem_err = min_tb[1][s.nflags];
                    if (s.stem > 0) {
        ...
        if (!two_staves && !two_dir)
            for (s = s1.next;; s = s.next) {
                ...
                switch (s.type) {
                    case REST:
                        g = s.ts_next;
                        if (!g || g.st != st || g.type != NOTE && g.type != REST) break;
                    case BAR:
                        if (s.invisible) break;
                    case CLEF:
                        y = a * s.x + b;
                        if (s1.stem > 0) {
    ...
    function draw_beams(bm) {
        ...
        da = 0;
        for (s = s1;; s = s.next) {
            if (s.type == NOTE && s.stem != beam_dir) s.ys = bm.a * s.xs + bm.b - staff_tb[s.st].y + bshift * (s.nflags - 1) * s.stem - bh;
            if (s == s2) break
        }
                ...
                if (k1 == k2) {
                    if (k1 == s1) {
                        ...
                        if (k.beam_br1 || k.beam_br2 && i > 2) {
                            ...
                            k1 = k1.prev;
                            while (k1.type != NOTE) k1 = k1.prev;
                            if (k1.nflags < k.nflags || k1.nflags == k.nflags && k1.dots < k.dots) x1 += bstub;
                            else x1 -= bstub
                        }
                    }
                }
    ...
    function draw_lstaff(x) {
        ...
        function draw_sysbra(x, st, flag) {
            ...
            i = st_end = st;
            while (1) {
                if (!cur_sy.staves[i].empty) st_end = i;
        ...
        for (i = 0;; i++) {
            ...
            if (!cur_sy.staves[i].empty) break;
            if (cur_sy.staves[i].flags & (CLOSE_BRACE | CLOSE_BRACKET)) l--;
            if (i == nst) break
        }
        for (j = nst; j > i; j--) {
            if (!cur_sy.staves[j].empty) break
        }
        if (i == j && l == 0) return;
    ...
    function draw_meter(x, s) {
        ...
        for (i = 0; i < s.a_meter.length; i++) {
            var meter = s.a_meter[i],
                f;
            if (meter.top == "C|") dx = 13;
            else dx = 13 * meter.top.length;
            if (meter.bot) {
                if (meter.bot.length > meter.top.length) dx = 13 * meter.bot.length;
    ...
    function draw_hl(x, yl, yu, st, hltype) {
        var i, p_staff = staff_tb[st],
            staffb = p_staff.y,
            stafflines = p_staff.stafflines,
            top = (stafflines.length - 1) * 2 * 3;
        if (top - p_staff.botline < 4) return;
        yl = Math.ceil(yl / 6) * 6;
        for (; yl < p_staff.botline; yl += 6) xygl(x, staffb + yl, hltype);
        yu -= yu % 6;
        for (; yu > top; yu -= 6) xygl(x, staffb + yu, hltype)
    }
    ...
    function draw_keysig(p_voice, x, s) {
        ...
        if (!s.k_a_acc) {
            if (cfmt.cancelkey || s.k_sf == 0) {
                if (s.k_sf == 0 || old_sf * s.k_sf < 0) {
                    ...
                    if (s.k_sf < old_sf) {
                        shift = sharp_cl[clef_ix];
                        p_seq = shift > 9 ? sharp1 : sharp2;
                        for (i = 0; i < s.k_sf; i++) shift += p_seq[i];
                        for (; i < old_sf; i++) {
                            xygl(x, staffb + shift, "acc3");
                            shift += p_seq[i];
                            x += 5.5
            ...
            for (i = 0; i < s.k_a_acc.length; i++) {
                ...
                if (i != 0 && (shift > last_shift + 18 || shift < last_shift - 18)) x -= 5.5;
    ...
    function draw_bar(s, bot, h) {
        ...
        for (i = bar_type.length; --i >= 0;) {
            switch (bar_type[i]) {
                case "|":
                    set_sscale(-1);
                    if (dotted) out_dotbar(x, bot, h);
                    else out_bar(x, bot, h);
                    break;
                default:
                    x -= 3;
                    set_sscale(-1);
    ...
    function draw_rest(s) {
        ...
        if (i >= 6) {
            ...
            switch (i) {
                case 6:
                    if (p_staff.stafflines[j] != "|") xygl(x, y + staffb, "hl");
                    break;
                case 7:
                    if (p_staff.stafflines[j + 1] != "|") xygl(x, y + 6 + staffb, "hl");
                    break;
                default:
                    if (p_staff.stafflines[j + 1] != "|") xygl(x, y + 6 + staffb, "hl");
                    if (i == 9) {
    ...
    function draw_gracenotes(s) {
        ...
        for (g = s.extra; g; g = g.next) {
            if (g.type != NOTE) continue;
            if (user.anno_start) anno_start(s);
            if (g.beam_st && !g.beam_end) {
                if (calculate_beam(bm, g)) draw_beams(bm)
            }
            draw_note(g, !bm.s2);
            if (g == bm.s2) bm.s2 = undefined;
            if (g.sappo) {
                if (!g.next) {
                    x1 = 9;
                    y1 = g.stem > 0 ? 5 : -5
            ...
            if (user.anno_stop) anno_stop(s);
            if (!g.next) break
        }
        if (voice_tb[s.v].key.k_bagpipe || !cfmt.graceslurs || s.slur_start || !s.next || s.next.type != NOTE) return;
        last = g;
        if (last.stem >= 0) {
            yy = 127;
            for (g = s.extra; g; g = g.next) {
    ...
    function draw_basic_note(x, s, m, y_tb) {
        var i, k, y, p, yy, no_head, dotx, doty, old_color = false,
            note = s.notes[m];
        staffb = staff_tb[s.st].y, y = 3 * (note.pit - 18), shhd = note.shhd * stv_g.scale;
        x_note = x + shhd;
        y_note = y + staffb;
        no_head = 0;
        if (note.a_dcn) {
        ...
        if (s.invisible) return;
        if (s.nohdi1 != undefined && m >= s.nohdi1 && m < s.nohdi2) return;
        ...
        if (no_head) {} else if (note.map && note.map[0]) {
            ...
            switch (head) {
                ...
                case "square":
                    ...
                    if (!tsnext && s.next && s.next.type == BAR && !s.next.next) dots = 0;
                    break;
                case "empty":
                    p = "Hd";
                    break;
        ...
        if (note.map && note.map[2]) old_color = set_color(note.map[2]);
        if (p) xygl(x_note, y_note, p);
        if (dots) {
    ...
    function next_scut(s) {
        ...
        for (s = s.next; s; s = s.next) {
            if (s.type == BAR && (s.bar_type[0] == ":" || s.bar_type == "|]" || s.bar_type == "[|" || s.text && s.text[0] != "1")) return s;
            prev = s
        }
        return prev
    }
    ...
    function slur_out(x1, y1, x2, y2, dir, height, dotted, st) {
        ...
        if (!dotted) output.push('<path class="fill" d="M');
        else output.push('<path class="stroke" stroke-dasharray="5,5" d="M');
        out_sxsy(x1, " ", y1);
    ...
    function draw_slur(k1, k2, m1, m2, slur_type) {
        ...
        x1 = k1.x;
        if (k1.notes) x1 += k1.notes[0].shhd;
        if (k1 != k2) {
            x2 = k2.x;
    ...
    function draw_slurs(first, last) {
        ...
        while (1) {
            ...
            if ((k.type == NOTE || k.type == REST || k.type == SPACE) && (k.slur_end || k.sl2)) {
                if (k.slur_end) {
                    ...
                    for (m2 = 0; m2 <= k.nhd; m2++)
                        if (k.notes[m2].sl2) break;
                    k.notes[m2].sl2--;
                    k.sl2--
                }
            } else {
                if (k.type != BAR || k.bar_type[0] != ":" && k.bar_type != "|]" && k.bar_type != "[|" && (!k.text || k.text[0] == "1")) cont = 1
            }
            ...
            if (s.slur_start || s.sl1) continue;
            if (s == last) break;
            s = s.next
        }
    }
    ...
    function draw_tuplet(t, s) {
        ...
        if (t.tuplet_f[0] == 1) return next;
        for (g = t.next; g; g = g.next) {
            if (g.type == TUPLET) {
                s3 = draw_tuplet(g, s);
                if (s3.time > next.time) next = s3
            }
        }
        r = t.tuplet_r;
        upstaff = s.st;
        for (s2 = s; s2; s2 = s2.next) {
            ...
            if (s2.type != NOTE && s2.type != REST) {
                if (s2.type == GRACE) {
                    for (g = s2.extra; g; g = g.next) {
                        if (g.type != NOTE) continue;
                        if (g.slur_start || g.sl1) some_slur = true
                    }
                }
                continue
            }
            if (s2.slur_start || s2.slur_end || s2.sl1 || s2.sl2) some_slur = true;
            if (s2.st < upstaff) upstaff = s2.st;
            if (!s1) s1 = s2;
            if (--r <= 0) break
        }
        if (!s2) return next;
        if (s2.time > next.time) next = s2;
        if (s1 == s2) {
            nb_only = true
            ...
            if (t.tuplet_f[0] == 2 || s1.type != NOTE || s2.type != NOTE) {
                ...
                for (s3 = s1;; s3 = s3.next) {
                    if (s3.type != NOTE && s3.type != REST) {
                        if (s3.type == GRACE || s3.type == SPACE) continue;
                        nb_only = false;
                        break
                    }
                    if (s3 == s2) break;
                ...
                if (nb_only && !s2.beam_end) {
                    for (s3 = s2.next; s3; s3 = s3.next) {
                        if (s3.type == NOTE || s3.type == REST) {
                            if (!s3.beam_br1 && !s3.beam_br2 && s3.nflags >= s2.nflags) nb_only = false;
                            break
                        }
                    }
                }
        ...
        if (nb_only) {
            if (t.tuplet_f[2] == 1) return next;
            xm = (s2.x + s1.x) * .5;
            if (s1 == s2) a = 0;
            else a = (s2.ys - s1.ys) / (s2.x - s1.x);
            b = s1.ys - a * s1.x;
            yy = a * xm + b;
            if (s1.stem > 0) {
            ...
            ym = a * xm + b;
            if (t.tuplet_f[2] == 0) out_bnum(xm, ym, t.tuplet_p);
            else out_bnum(xm, ym, t.tuplet_p + ":" + t.tuplet_q);
            if (s1.stem > 0) {
                ym += 10;
        ...
        if (some_slur) {
            draw_slurs(s1, s2);
            if (s1.slur_start || s1.sl1) return next;
            for (s3 = s1.next; s3 != s2; s3 = s3.next) {
                if (s3.slur_start || s3.slur_end || s3.sl1 || s3.sl2) return next
            }
            if (s2.slur_end || s2.sl2) return next
        }
        if (t.tuplet_f[1] != 0) error(2, t, "'what' value of %%tuplets not yet coded");
        if (s1.multi >= 0) {
            x1 = s1.x - 4;
            y1 = 24;
            if (s1.st == upstaff) {
        ...
        yy = .5 * (y1 + y2);
        if (t.tuplet_f[2] == 0) out_bnum(xm, yy, t.tuplet_p, true);
        else out_bnum(xm, yy, t.tuplet_p + ":" + t.tuplet_q, true);
        if (s1.multi >= 0) {
            yy += 9;
        ...
        for (i = 0; i < mhead1.length; i++) {
            ...
            if (dir > 0) {
                if (m1 < k1.nhd && p1 + 1 == k1.notes[m1 + 1].pit)
                    if (k1.notes[m1 + 1].shhd > sh) sh = k1.notes[m1 + 1].shhd
            } else {
            ...
            if (dir > 0) {
                if (m2 < k2.nhd && p2 + 1 == k2.notes[m2 + 1].pit)
                    if (k2.notes[m2 + 1].shhd < sh) sh = k2.notes[m2 + 1].shhd
            } else {
    ...
    function draw_ties(k1, k2, job) {
        ...
        if (job == 2) {
            for (i = 0; i <= nh1; i++) {
                if (k1.notes[i].ti1) mhead3.push(i)
            }
            draw_note_ties(k1, k2 || k1, mhead3, mhead3, job);
            return
        }
        for (i = 0; i <= nh1; i++) {
            if (k1.notes[i].ti1 == 0) continue;
            tie2 = -1;
            pit = k1.notes[i].apit;
            for (m1 = k2.nhd; m1 >= 0; m1--) {
                switch (k2.notes[m1].apit - pit) {
                    case 1:
                    case -1:
                        if (k1.notes[i].acc != k2.notes[m1].acc) tie2 = m1;
                    default:
                        continue;
                    case 0:
                        tie2 = m1;
        ...
        if (mhead3.length != 0) error(1, k1, "Bad tie")
    }
    ...
    function draw_all_ties(p_voice) {
        ...
        for (s2 = s1; s2; s2 = s2.next) {
            if (s2.dur || s2.type == GRACE) break;
            if (s2.type != BAR || !s2.text) continue;
            if (s2.text[0] == "1") s_rtie = p_voice.s_tie;
            else p_voice.s_tie = s_rtie
        }
        if (!s2) return;
        if (p_voice.s_tie) {
        ...
        while (1) {
            for (s1 = s2; s1; s1 = s1.next) {
                if (s1.ti1) break;
                if (!s_rtie) continue;
                if (s1.type != BAR || !s1.text) continue;
                if (s1.text[0] == "1") {
                    s_rtie = null;
                    continue
                }
                if (s1.bar_type == "|") continue;
            ...
            if (!s2) {
                for (s4 = s1.ts_next; s4; s4 = s4.ts_next) {
                    if (s4.st != s1.st) continue;
                    if (s4.time < time) continue;
                    if (s4.time > time) {
                        s4 = null;
                        break
                    }
                ...
                if (s2.time == time) s4 = s2;
                else s4 = tie_comb(s1)
            }
            for (s3 = s1.ts_next; s3; s3 = s3.ts_next) {
                if (s3.st != s1.st) continue;
                if (s3.type == CLEF) {
                    clef_chg = true;
                    continue
                }
                if (s3.time > time) break;
                if (s3.type == BAR) {
    ...
    function draw_all_slurs(p_voice) {
        ...
        for (; s; s = s.next) {
            if (s.type != NOTE && s.type != REST && s.type != SPACE) continue;
            while (s.slur_end || s.sl2) {
                ...
                if (k.type != BAR || k.bar_type[0] != ":" && k.bar_type != "|]" && k.bar_type != "[|" && (!k.text || k.text[0] == "1")) slur_st >>= 4
            }
        }
        s = p_voice.sym;
        while (slur_st != 0) {
    ...
    function draw_vname(indent) {
        ...
        for (st = cur_sy.nstaff; st >= 0; st--) {
            if (!cur_sy.staves[st].empty) break
        }
        if (st < 0) return;
        for (v = 0; v < voice_tb.length; v++) {
            p_voice = voice_tb[v];
            ...
            if (!cur_sy.staves[st]) continue;
            if (cur_sy.staves[st].empty) continue;
            if (p_voice.new_name) {
            ...
            if (cur_sy.staves[st].flags & CLOSE_BRACE2) {
                while (!(cur_sy.staves[st].flags & OPEN_BRACE2)) st--
            } else if (cur_sy.staves[st].flags & CLOSE_BRACE) {
                while (!(cur_sy.staves[st].flags & OPEN_BRACE)) st--
            }
        ...
        for (st = 0; st < staff_d.length; st++) {
            ...
            if (cur_sy.staves[st].flags & OPEN_BRACE2) {
                while (!(cur_sy.staves[n].flags & CLOSE_BRACE2)) n++
            } else if (cur_sy.staves[st].flags & OPEN_BRACE) {
                while (!(cur_sy.staves[n].flags & CLOSE_BRACE)) n++
            }
            if (n != st) y -= (staff_tb[st].y - staff_tb[n].y) * .5;
    ...
    function set_staff() {
        ...
        for (st = 0; st < sy.staves.length; st++) {
            if (!sy.staves[st].empty) empty[st] = false
        }
        for (s = tsfirst; s; s = s.ts_next) {
            if (!s.new_sy) continue;
            sy = sy.next;
            for (st = 0; st < sy.staves.length; st++) {
                if (!sy.staves[st].empty) empty[st] = false
            }
        ...
        for (v = 0; v < voice_tb.length; v++) {
            p_voice = voice_tb[v];
            if (p_voice.scale != 1) p_voice.scale_str = 'transform="scale(' + p_voice.scale.toFixed(2) + ')"'
        }
        for (st = 0; st <= nstaff; st++) {
            if (!empty[st]) break;
        ...
        for (st++; st <= nstaff; st++) {
            ...
            if (sy_staff_prev.sep) staffsep = sy_staff_prev.sep;
            else staffsep = cfmt.sysstaffsep;
            if (sy_staff_prev.maxsep) maxsep = sy_staff_prev.maxsep;
        ...
        if (dy > maxsep) dy = maxsep;
        y += dy;
        if (y > cfmt.maxstaffsep) y = cfmt.maxstaffsep;
        return y
    }
    ...
    function bar_set(bar_bot, bar_height) {
        var bar_bot = [],
            bar_height = [],
            st, nlines, staffscale, top, bot, dy = 0;
    ...
    function draw_systems(indent) {
        ...
        function draw_staff(st, x1, x2) {
            var w, i, dx, y = staff_tb[st].y,
                stafflines = staff_tb[st].stafflines;
            if (stafflines[stafflines.length - 1] == ".") return;
            set_sscale(st);
            w = (x2 - x1) / stv_g.scale;
            for (i = 0; i < stafflines.length; i++) {
                if (stafflines[i] != ".") break;
                y += 6
            }
            xypath(x1, y);
        ...
        draw_vname(indent);
        for (st = 0; st <= nstaff; st++) xstaff[st] = !cur_sy.staves[st] || cur_sy.staves[st].empty ? -1 : 0;
        res = bar_set();
        var bar_bot = res[0],
            bar_height = res[1];
        draw_lstaff(0);
        for (s = tsfirst; s; s = s.ts_next) {
                ...
                for (st = 0; st <= nstaff; st++) {
                    ...
                    if (x < 0) {
                        if (next_sy.staves[st] && !next_sy.staves[st].empty) {
                            if (s.type == BAR) xstaff[st] = s.x;
                            else xstaff[st] = s.x - s.wl - 2
                        }
                        continue
                    }
                    if (s.ts_prev.type == BAR) x2 = s.ts_prev.x;
                    else x2 = s.x - s.wl - 2;
                    if (!next_sy.staves[st] || next_sy.staves[st].empty) {
                        xstaff[st] = -1
            ...
            switch (s.type) {
                case BAR:
                    if (s.second || cur_sy.staves[st].empty) s.invisible = true;
                    if (s.invisible) break;
                    if (user.anno_start) anno_start(s);
                    ...
                    if (cur_sy.voices[s.v].range == 0) {
                        if (s.xmx > .5 * CM) {
                            ...
                            for (var i = 0; i < voice_tb.length; i++) {
                                if (cur_sy.voices[i].range > 0) nvoice++
                            }
                            for (s2 = s.ts_next; s2; s2 = s2.ts_next) {
                                if (s2.type != STBRK) break;
                                nvoice--
                            }
                            if (nvoice == 0) draw_lstaff(s.x)
                        }
                    }
                    s2 = s.prev;
                    if (!s2) break;
                    x2 = s2.x;
                    if (s2.type != BAR) x2 += s2.wr;
                    st = s.st;
                    x = xstaff[st];
                    if (x >= 0) {
                        if (x >= x2) continue;
    ...
    function draw_symbols(p_voice) {
        ...
        for (s = p_voice.sym; s; s = s.next) {
            ...
            if (s.invisible && s.type != NOTE && s.type != REST && s.type != GRACE) continue;
            x = s.x;
            switch (s.type) {
                case NOTE:
                    set_color(p_voice.color);
                    ...
                    if (s.beam_st && !s.beam_end || first_note && !s.beam_st) {
                        first_note = false;
                        if (calculate_beam(bm, s)) draw_beams(bm)
                    }
                    if (user.anno_start) anno_start(s);
                    draw_note(s, !bm.s2);
                    if (user.anno_stop) anno_stop(s);
                ...
                case CLEF:
                    st = s.st;
                    if (s.second) break;
                    if (s.invisible || staff_tb[st].empty) break;
                    set_color(undefined);
                    set_sscale(st);
                    if (user.anno_start) anno_start(s);
                    y = staff_tb[st].y;
                ...
                case METER:
                    p_voice.meter = s;
                    if (s.second || staff_tb[s.st].empty) break;
                    if (cfmt.alignbars && s.st != 0) break;
                    set_color(undefined);
                    set_sscale(s.st);
                    if (user.anno_start) anno_start(s);
                    draw_meter(x, s);
    ...
    function draw_all_sym() {
        ...
        for (v = 0; v < n; v++) {
            p_voice = voice_tb[v];
            if (p_voice.sym) draw_symbols(p_voice)
        }
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.type == CLEF) staff_tb[s.st].clef = s
        }
    }
        ...
        for (s = sym; s; s = s.next) {
            ...
            if (ntie <= 1) {
                ...
                for (i = 0; i <= s.nhd; i++) {
                    ti = s.notes[i].ti1;
                    if (ti != 0) {
                        if ((ti & 7) == SL_AUTO) s.notes[i].ti1 = ti & SL_DOTTED | dir;
                        break
                    }
                }
                continue
            ...
            if (sec == 0) {
                if (ntie & 1) {
                    ...
                    for (i = 0; i <= s.nhd; i++) {
                        ...
                        if (ti == 0) continue;
                        if (ntie == 0) {
                            if (s.notes[i].pit >= 22) dir = SL_ABOVE
                        }
                        if ((ti & 7) == SL_AUTO) s.notes[i].ti1 = ti & SL_DOTTED | dir;
                        if (ntie-- == 0) dir = SL_ABOVE
                    }
                    continue
                }
                ntie /= 2;
    ...
    function set_tie_room() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            for (; s; s = s.next) {
                ...
                s2 = s.next;
                while (s2 && s2.type != NOTE) s2 = s2.next;
                if (s2) {
    ...
    function param_set_font(xxxfont, param) {
        ...
        if (fn) {
            font = font_tb[fn];
            if (font) old_fn = font.name + "." + font.size
        }
        a = param.split(/\s+/);
        new_name = a[0];
        if (new_name == "*" && font) {
        ...
        if (a.length > 1) {
            ...
            if (new_size == "*") new_size = font.size
        } else {
            new_size = font.size
        }
    ...
    function set_posit(k, a) {
        var v, val, posit;
        if (a[0] >= "0" && a[0] <= "9") val = parseInt(a);
        else val = get_posit(a);
        if (curvoice) posit = curvoice.posit;
        else posit = cfmt.posit;
        switch (k) {
            case "dynamic":
                k = "dyn";
                break;
        ...
        posit[k] = val;
        if (!curvoice) {
            for (v = 0; v < voice_tb.length; v++) voice_tb[v].posit[k] = val
        }
    }
    ...
    function set_format(cmd, param, lock) {
        ...
        switch (cmd) {
            ...
            case "microscale":
                ...
                if (parse.state == 1) {
                    for (v = 0; v < voice_tb.length; v++) voice_tb[v].microscale = cfmt.microscale
                } else {
            ...
            case "staffwidth":
                ...
                if (rmargin < 0) parse.line.error("'staffwidth' too big");
                else cfmt.rightmargin = rmargin;
                break;
            case "textoption":
    ...
    function tosvg(in_fname, file) {
        ...
        function tune_selected() {
            ...
            i = file.indexOf("\n", i);
            if (parse.select.test(file.slice(bol, i))) return true;
            eol = file.indexOf("\n\n", i);
            if (eol < 0) eol = eof - 1;
            else eol++;
            return false
        ...
        function uncomment(src, do_escape) {
            var i, j, c, l;
            if (do_escape && src.indexOf("\\") >= 0) src = cnv_escape(src);
            l = src.length;
            for (i = 0; i < l; i++) {
                c = src[i];
                switch (c) {
        ...
        for (bol = 0;; bol = eol + 1) {
            ...
            if (eol == bol) {
                ...
                } else if (parse.state >= 2) end_tune();
                continue
            }
            ...
            if (pscom) {
                ...
                switch (a[0]) {
                    ...
                    case "voice":
                        ...
                        if (!select) {
                            if (parse.cur_tune_opts) parse.cur_tune_opts.voice_opts = null;
                            else parse.voice_opts = null;
                            continue
                        }
                        if (select == "end") continue;
                        if (parse.cur_tune_opts) {
                            if (!parse.cur_tune_opts.voice_opts) parse.cur_tune_opts.voice_opts = {};
            ...
            switch (line0) {
                case "X":
                    ...
                    if (parse.select && !tune_selected()) continue;
                    cfmt_sav = clone(cfmt);
                ...
                case "K":
                    if (parse.state == 0) continue;
                    if (parse.state == 1) info.K = text;
                    else if (parse.state == 2) goto_tune();
                    parse.line.buffer = text;
                    parse.line.index = 0;
                    do_info(line0, text);
                    if (parse.state != 1) continue;
                    parse.state = 2;
                    if (!glovar.ulen) glovar.ulen = BASE_LEN / 8;
                    continue;
                case "W":
                    if (parse.state == 0 || cfmt.writefields.indexOf("W") < 0) break;
                    if (!info.W) info.W = text;
                    else info.W += txt_add + text;
                    break;
                case "s":
                    if (parse.state != 3) break;
                    break;
                case "w":
                    if (parse.state != 3 || cfmt.writefields.indexOf("w") < 0) break;
    ...
    var space_tb = [7, 10, 14.15, 20, 28.3, 40, 56.6, 80, 113, 150],
        smallest_duration;
    const dx_tb = {
        full: 9,
        empty: 10,
    ...
    function set_head_directions(s) {
        var i, i1, i2, d, ps, dx, dx_head = dx_tb[s.head],
            dir = s.stem,
            n = s.nhd;
        if (s.dur >= BASE_LEN * 2 && s.head == "oval") dx_head = 15.8;
        if (n == 0) {
            if (s.notes[0].acc) {
                if (s.grace) dx_head *= .7;
    ...
    function set_acc_shft() {
        ...
        while (s) {
            ...
            for (s2 = s; s2; s2 = s2.ts_next) {
                if (s2.time != t || s2.type != NOTE || s2.st != st) break;
                if (acc) continue;
                for (i = 0; i <= s2.nhd; i++) {
                    if (s2.notes[i].acc) {
                        acc = true;
    ...
    function unlksym(s) {
        ...
        if (s.ts_next) {
            ...
            if (s.new_sy) s.ts_next.new_sy = true;
    ...
    function may_combine(s) {
        ...
        if (!s2 || s2.type != NOTE && s2.type != REST) return false;
        if (s2.v == s.v || s2.st != s.st || s2.time != s.time || s2.dur != s.dur) return false;
        if (s.combine <= 0 && s2.type != s.type) return false;
        if (s.a_gch && s2.a_gch) return false;
        if (s.type == REST) {
            if (s.type == s2.type && s.invisible && !s2.invisible) return false;
            return true
        }
        if (s2.a_ly || s2.sl1 || s2.sl2 || s2.slur_start || s2.slur_end) return false;
        if (s2.beam_st != s.beam_st || s2.beam_end != s.beam_end) return false;
        nhd2 = s2.nhd;
        if (s.combine <= 1 && s.notes[0].pit <= s2.notes[nhd2].pit + 1) return false;
        return true
    }
    ...
    function combine_notes(s, s2) {
        ...
        if (s.combine >= 3) {
            for (m = nhd; m > 0; m--) {
                if (s.notes[m].pit == s.notes[m - 1].pit && s.notes[m].acc == s.notes[m - 1].acc) s.notes.splice(m, 1)
            }
            s.nhd = nhd = s.notes.length - 1
        }
        ...
        s.yav = (s.ymx + s.ymn) / 2;
        if (s.a_dd) {
            if (s2.a_dd) s.a_dd = s.a_dd.concat(s2.a_dd)
        } else {
            s.a_dd = s2.a_dd
        }
        type = s.notes[0].ti1;
        if ((type & 15) == SL_AUTO) s.notes[0].ti1 = SL_BELOW | type & ~SL_DOTTED;
        type = s.notes[nhd].ti1;
        if ((type & 15) == SL_AUTO) s.notes[nhd].ti1 = SL_ABOVE | type & ~SL_DOTTED
    }
    ...
    function do_combine(s) {
        ...
        while (1) {
            ...
            if (s.type != s2.type) {
                ...
                if (s.invisible && !s2.invisible) delete s.invisible
            } else {
                combine_notes(s, s2)
            }
            if (s2.text && !s.text) {
            ...
            unlksym(s2);
            if (s.in_tuplet || !may_combine(s)) break
        }
    }
    ...
    function combine_voices() {
        ...
        for (var s = tsfirst; s.ts_next; s = s.ts_next) {
            if (s.combine < 0) continue;
            if (s.combine == 0 && s.type != REST) continue;
            if (s.in_tuplet) {
                g = s.extra;
                if (!g) continue;
                r = 0;
                for (; g; g = g.next) {
                    if (g.type == TUPLET && g.tuplet_r > r) r = g.tuplet_r
                }
                if (r == 0) continue;
                i = r;
                for (s2 = s; s2; s2 = s2.next) {
                    if (!s2.ts_next) break;
                    if (s2.type != NOTE && s2.type != REST) continue;
                    if (!may_combine(s2)) break;
                    if (--i <= 0) break
                }
                if (i > 0) continue;
                for (s2 = s;; s2 = s2.next) {
            ...
            if (s.type != NOTE) {
                if (s.type == REST) {
                    if (may_combine(s)) do_combine(s)
                }
                continue
            }
            if (!s.beam_st) continue;
            if (s.beam_end) {
                if (may_combine(s)) do_combine(s);
                continue
            }
    ...
    function insert_clef(s, clef_type, clef_line) {
        var p_voice = voice_tb[s.v],
            new_s, st = s.st;
        if (s.type == BAR && s.prev && s.prev.type == BAR) s = s.prev;
        ...
        if (s.ts_prev.type != CLEF) new_s.seqst = true;
    ...
    function set_graceoffs(s) {
        ...
        for (;; g = g.next) {
            ...
            if (g.nflags <= 0) xx += gspinside / 4;
            if (g.y > next.y + 8) xx -= 1.5;
            xx += gspinside
        }
        xx += gspleft + gspright;
        next = s.next;
        if (next && next.type == NOTE) {
            if (g.y >= 3 * (next.notes[next.nhd].pit - 18)) xx -= 1;
            else if (g.beam_st && g.y < 3 * (next.notes[0].pit - 18) - 7) xx += 2
        }
        return xx
    }

    function gchord_width(s, wlnote, wlw) {
        var s2, gch, w, wl, lspc = 0,
            rspc = 0;
        ...
        switch (s.type) {
            ...
            case REST:
                ...
                if (s.dots > 0) {
                    ...
                    if (s.wr < s.xmx + 12) s.wr = s.xmx + 12;
                ...
                if (s2 && s2.type == GRACE) s.wl = wlnote - 4.5;
                else s.wl = wlw;
                return;
            case SPACE:
                xx = s.width / 2;
            ...
            case BAR:
                ...
                if (!s.invisible) {
                    ...
                    s.wl = w;
                    if (s.next && s.next.type != METER) s.wr = 8;
                    else s.wr = 5
                } else {
                    s.wl = s.wr = 0
                }
                if (s.a_dd) s.wl += deco_width(s);
                if (s.a_gch && s.a_gch[0].text.length < 4) s.wl = gchord_width(s, s.wl, s.wl);
                return;
            case CLEF:
                if (s.invisible) break;
                s.wl = 12;
            ...
            case KEY:
                ...
                if (!s.k_a_acc) {
                    ...
                    for (i = 1; i < n2; i++) {
                        acc = s.k_a_acc[i];
                        if (acc.pit > s.k_a_acc[i - 1].pit + 6 || acc.pit < s.k_a_acc[i - 1].pit - 6) n1--;
            ...
            case METER:
                ...
                for (i = 0; i < s.a_meter.length; i++) {
                    ...
                    if (meter.top == "C|") {
                        ...
                        if (!meter.bot || meter.top.length > meter.bot.length) w += 6.5 * meter.top.length;
                        else w += 6.5 * meter.bot.length
                    }
                }
                s.wl = w;
                s.wr = w + 7;
    ...
    function set_space(s) {
            ...
            switch (s.type) {
                ...
                case REST:
                    if (s.ts_prev.type == BAR) {
                        if (s.nflags < -2) return space_tb[0];
                        return space_tb[2]
                    }
                    break
            }
        ...
        if (s.ts_prev.type == MREST) return s.ts_prev.wr + 16 + 3;
        if (smallest_duration >= BASE_LEN / 2) {
            if (smallest_duration >= BASE_LEN) len /= 4;
            else len /= 2
        }
        if (len >= BASE_LEN / 4) {
            if (len < BASE_LEN / 2) i = 5;
            else if (len < BASE_LEN) i = 6;
            else if (len < BASE_LEN * 2) i = 7;
            else if (len < BASE_LEN * 4) i = 8;
            else i = 9
        } else {
            if (len >= BASE_LEN / 8) i = 4;
            else if (len >= BASE_LEN / 16) i = 3;
            else if (len >= BASE_LEN / 32) i = 2;
            else if (len >= BASE_LEN / 64) i = 1;
            else i = 0
        }
        ...
        if (!s.dur) {
            if (s.type == BAR) {
                if (s.bar_type.length > 1) space *= .8;
                else space *= .7
            }
            return space
        }
    ...
    function set_allsymwidth(last_s) {
        ...
        while (1) {
            set_width(s);
            if (new_val < s.wl) new_val = s.wl;
            s = s.ts_next;
            if (s == last_s || s.seqst) break
        }
        ...
        while (1) {
            ...
            do {
                ...
                for (s3 = s.ts_prev; s3; s3 = s3.ts_prev) {
                    if (new_val < s3.wr && (s3.type == NOTE || s3.type == REST) && (s2.type == NOTE || s2.type == REST)) new_val = s3.wr;
                ...
                if (shrink < new_val) shrink = new_val;
                new_val = set_space(s2);
                if (space < new_val) space = new_val;
                if ((s2 = s2.ts_next) == last_s) break
            } while (!s2.seqst);
    ...
    function set_repeat(g, s) {
        ...
        if ((n = g.repeat_n) < 0) {
            ...
            for (j = g.repeat_k; --j >= 0;) {
                ...
                s3.beam_st = true;
                set_width(s3);
                if (s3.seqst) s3.space = set_space(s3);
                s3.head = "square";
                s3 = s2
            }
            g.fmt_type = undefined;
        ...
        if (n == 2) {
            ...
            if (s3.seqst) s3.space = set_space(s3);
            s2.bar_mrep = 2;
            if (s2.seqst) s2.space = set_space(s2);
            s3 = s2.next;
            s2 = s3.next;
            while (1) {
                if (s2.type == BAR || s2.type == CLEF) break;
                delete s2.extra;
                unlksym(s2);
                s2 = s2.next
            }
    ...
    function set_nl(s) {
        ...
        if (s.eoln && !cfmt.keywarn && !cfmt.timewarn) return set_eol_next(s);
        switch (s.type) {
            case CLEF:
            case BAR:
                break;
            case KEY:
                if (cfmt.keywarn && !s.k_none) break;
        ...
        for (;; s = s.ts_next) {
            ...
            switch (s.type) {
                case BAR:
                    if (done || !s.bar_num && s.next && s.bar_type[s.bar_type.length - 1] == ":" && s.bar_type[0] != ":") cut_here = true;
                    else done = 1;
                    break;
                case STBRK:
                    if (!s.stbrk_forced) {
                    ...
                    break;
                case METER:
                    if (!cfmt.timewarn) cut_here = true;
                    break;
                case CLEF:
                    if (done) cut_here = true;
                    break;
                case KEY:
                    if (!cfmt.keywarn || s.k_none) cut_here = true;
                    break;
                default:
                    if (!done || s.prev && s.prev.type == GRACE) break;
                    cut_here = true;
                    break
            }
            if (cut_here) break;
            if (s.extra) {
                if (!extra) extra = s;
                else error(2, s, "Extra symbol may be misplaced")
            }
        }
        if (extra && extra != s) {
            s2 = extra.extra;
    ...
    function set_lines(first, last, lwidth, indent) {
        ...
        for (s = first; s != last; s = s.ts_next) {
            ...
            s.x = wwidth;
            shrink = s.shrink;
            if ((space = s.space) < shrink) wwidth += shrink;
            else wwidth += shrink * cfmt.maxshrink + space * (1 - cfmt.maxshrink)
        }
        s = first;
        while (1) {
            ...
            if (!cut_here) {
                ...
                for (; s != last; s = s.ts_next) {
                    ...
                    x = s.x;
                    if (!x || x < xmin) continue;
                    if (x + x.shrink >= xmax) break;
                    if (beam != 0) continue;
                    s2 = s;
                    if ((s.time - bar_time) % (BASE_LEN / 8) == 0) s3 = s
                }
                if (s3) s2 = s3;
                if (s2) s = s2;
                while (!s.x || s.x + s.shrink * 2 >= xmax) s = s.ts_prev
            }
            if (s.nl) {
    ...
    function set_auto_clef(st, s_start, clef_type_start) {
        ...
        for (s = s_start; s != s_last; s = s.ts_next) {
            if (s.new_sy && s != s_start) break;
            if (s.st != st || s.type != NOTE) continue;
            time = s.time;
            if (clef_type == "t") {
                ...
                s2 = s.ts_prev;
                if (s2 && s2.time == time && s2.st == st && s2.type == NOTE && s2.notes[0].pit >= 19) continue;
                s2 = s.ts_next;
                if (s2 && s2.st == st && s2.time == time && s2.type == NOTE && s2.notes[0].pit >= 19) continue
            } else {
                if (s.notes[0].pit < 12 || s.notes[s.nhd].pit < 20) {
                ...
                s2 = s.ts_prev;
                if (s2 && s2.time == time && s2.st == st && s2.type == NOTE && s2.notes[0].pit <= 13) continue;
                s2 = s.ts_next;
                if (s2 && s2.st == st && s2.time == time && s2.type == NOTE && s2.notes[0].pit <= 13) continue
            }
            if (!s_last_chg) {
            ...
            for (s2 = s.ts_prev; s2 != s_last_chg; s2 = s2.ts_prev) {
                ...
                if (s2.type != NOTE) continue;
                if (s2.beam_st && !voice_tb[s2.v].second) s3 = s2
            }
    ...
    function set_clefs() {
        var s, s2, st, v, p_voice, g, new_type, new_line, p_staff, staff_clef = new Array(nstaff),
            sy = cur_sy;
        ...
        for (v = 0; v < voice_tb.length; v++) {
            p_voice = voice_tb[v];
            if (sy.voices[v].range < 0) continue;
            st = sy.voices[v].st;
            if (!sy.voices[v].second) {
                if (p_voice.stafflines != undefined) sy.staves[st].stafflines = p_voice.stafflines;
            ...
            if (!sy.voices[v].second && !p_voice.clef.clef_auto) staff_clef[st].autoclef = false
        }
        for (v = 0; v < voice_tb.length; v++) {
            p_voice = voice_tb[v];
            if (sy.voices[v].range < 0 || sy.voices[v].second) continue;
            st = sy.voices[v].st;
            s = p_voice.clef;
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            if (s.new_sy) {
                ...
                for (v = 0; v < voice_tb.length; v++) {
                    ...
                    if (new_type == staff_clef[st].clef.clef_type && new_line == staff_clef[st].clef.clef_line) continue;
                    g = s;
                    while (g.v != v) g = g.ts_next;
                    if (g.type != CLEF) {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            for (s = s2; s; s = s.next) s.notes[0].pit = pitch
        }
    }
    const delta_tb = {
        t: 0 - 2 * 2,
    ...
    function set_pitch(last_s) {
        var s, s2, g, st, delta, m, pitch, dur, note, staff_delta = new Array(nstaff),
            sy = cur_sy;
        for (st = 0; st <= nstaff; st++) {
            s = staff_tb[st].clef;
        ...
        for (s = tsfirst; s != last_s; s = s.ts_next) {
            ...
            switch (s.type) {
                ...
                case NOTE:
                    ...
                    s.ymn = 3 * (s.notes[0].pit - 18) - 4;
                    s.yav = (s.ymx + s.ymn) / 2;
                    if (s.type != NOTE) s.y = Math.floor(s.yav / 6) * 6;
    ...
    function set_stem_dir() {
        var t, u, i, st, rvoice, v, v_s, st_v, vobj, v_st_tb, st_v_tb = [],
            s = tsfirst,
            sy = cur_sy,
            nst = sy.nstaff;
        while (s) {
            ...
            for (u = s; u; u = u.ts_next) {
                ...
                if (u.type != NOTE && u.type != REST || u.invisible) continue;
                st = u.st;
                if (st > nst) {
            ...
            for (; s != u; s = s.ts_next) {
                if (s.multi) continue;
                if (s.type != NOTE && s.type != REST && s.type != GRACE) continue;
                st = s.st;
                v = s.v;
                v_st = v_st_tb[v];
                st_v = st_v_tb[st];
                ...
                if (i == st_v.length - 1) {
                    ...
                    if (i != 0 && i + 1 == st_v.length - 1) {
                        ...
                        t = s.ts_next;
                        if (s.ts_prev && s.ts_prev.time == s.time && s.ts_prev.st == s.st && s.notes[s.nhd].pit == s.ts_prev.notes[0].pit && s.beam_st && s.beam_end && (!t || t.st != s.st || t.time != s.time)) s.multi = -1
                    }
                }
            }
            while (s && s.type == BAR) {
    ...
    function shift_rest(s, s2, sy) {
        var y, us = rest_sp[5 - s.nflags][0],
            ls = rest_sp[5 - s.nflags][1];
        var ymx = s.y + us,
            ymn = s.y - ls;
        if (ymx < s2.ymn || ymn > s2.ymx) return;
        if (sy.voices[s.v].range > sy.voices[s2.v].range) ymx = s2.ymn;
        else ymx = s2.ymx;
        if (ymx >= s2.ymx) {
    ...
    function set_rest_offset() {
        var s, s2, prev, v, end_time, not_alone, v_s, y, v_s_tb = [],
            sy = cur_sy;
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.invisible) continue;
            if (s.new_sy) sy = sy.next;
            switch (s.type) {
                default:
                    continue;
                case NOTE:
            ...
            for (v = 0; v <= v_s_tb.length; v++) {
                ...
                if (v < s.v && v_s.s.time == s.time) prev = v.s;
                break
            }
            end_time = s.time + s.dur;
            for (s2 = s.ts_next; s2; s2 = s2.ts_next) {
                if (s2.time >= end_time) break;
                if (s2.st != s.st || s2.type != NOTE && s2.type != REST || s2.invisible) continue;
                not_alone = true;
                if (s2.type != REST) {
                    shift_rest(s, s2, sy);
    ...
    function new_sym(type, p_voice, last_s) {
        ...
        s.ts_prev.ts_next = s;
        if (s.ts_prev.type != type) s.seqst = true;
        last_s.ts_prev = s;
    ...
    function init_music_line() {
        ...
        for (v = 0; v < nv; v++) {
            p_voice = voice_tb[v];
            if (cur_sy.voices[v].range < 0) continue;
            p_voice.second = cur_sy.voices[v].second;
            st = cur_sy.voices[v].st;
            while (st < nstaff && cur_sy.staves[st].empty) st++;
            p_voice.st = st
        }
        last_s = tsfirst;
        ...
        for (v = 0; v < nv; v++) {
            p_voice = voice_tb[v];
            if (p_voice.sym && p_voice.sym.type == CLEF) continue;
            if (cur_sy.voices[v].range < 0 || cur_sy.voices[v].second && !p_voice.bar_start) continue;
            st = cur_sy.voices[v].st;
            ...
            last_s.ts_prev = s;
            if (last_s.type == CLEF) delete last_s.seqst;
            delete s.clef_small;
            s.second = cur_sy.voices[v].second;
            if (cur_sy.staves[st].empty) s.invisible = true
        }
        for (v = 0; v < nv; v++) {
            if (cur_sy.voices[v].range < 0 || cur_sy.voices[v].second || cur_sy.staves[cur_sy.voices[v].st].empty) continue;
            p_voice = voice_tb[v];
        ...
        if (insert_meter & 1) {
            for (v = 0; v < nv; v++) {
                p_voice = voice_tb[v];
                if (cur_sy.voices[v].range < 0 || cur_sy.voices[v].second || cur_sy.staves[cur_sy.voices[v].st].empty || p_voice.meter.text == "none") continue;
        ...
        for (v = 0; v < nv; v++) {
            p_voice = voice_tb[v];
            if (!p_voice.bar_start || cur_sy.voices[v].range < 0 || cur_sy.staves[cur_sy.voices[v].st].empty) continue;
            i = 2;
            if (!p_voice.bar_start.text && p_voice.bar_start.bar_type == "[") {
                for (s = p_voice.last_sym; s; s = s.next) {
                    ...
                    if (s.type != BAR) continue;
                    if (s.bar_type.length > 1 || s.bar_type == "]" || s.text) break;
                    if (--i < 0) break
                }
                if (!s) i = -1;
                if (i >= 0 && p_voice.last_sym.time == s.time) i = -1
            }
            if (i >= 0) {
                ...
                if (p_voice.bar_start.invisible) s.invisible = true;
                if (p_voice.bar_start.norepbra) s.norepbra = true;
            ...
            for (s = sym; s; s = s.next) {
                switch (s.type) {
                    ...
                    case BAR:
                        ...
                        if (!s.next && s.prev && s.prev.type == NOTE && s.prev.dur >= BASE_LEN * 2) s.prev.head = "square";
                        break;
                    case NOTE:
                    case REST:
                        if (s.trem2) break;
    ...
    function set_indent() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            while (1) {
                j = p.indexOf("\\n", i);
                if (j < 0) w = strw(p.slice(i));
                else w = strw(p.slice(i, j));
                if (w > maxw) maxw = w;
                if (j < 0) break;
                i = j + 1
            }
        ...
        if (maxw != 0) {
            ...
            for (st = 0; st <= cur_sy.nstaff; st++) {
                ...
                if (cur_sy.staves[st].flags & (OPEN_BRACE | OPEN_BRACKET) && w == 0) w = 10
            }
            maxw += 4 * cwid(" ") * font.swfac + w
        }
        if (insert_meter & 2) maxw += cfmt.indent;
        return maxw
    }

    function set_beams(sym) {
        var s, t, g, beam, s_opp, dy, laststem = -1,
            lasty = 0;
        for (s = sym; s; s = s.next) {
            if (s.type != NOTE) {
                if (s.type != GRACE) continue;
                g = s.extra;
                while (g.type != NOTE) g = g.next;
                if (g.stem == 2) {
                    s_opp = s;
                    continue
                }
                if (s.stem == 0 && (s.stem = s.multi) == 0) s.stem = 1;
                for (; g; g = g.next) {
                    g.stem = s.stem;
                    g.multi = s.multi
                }
            ...
            if (s.stem == 0 && (s.stem = s.multi) == 0) {
                if (beam) {
                    ...
                    if (avg < n) laststem = 1;
                    else if (avg > n || cfmt.bstemdown) laststem = -1;
                    beam = true;
                    s.stem = laststem
                } else {
                    s.stem = s.yav >= 12 ? -1 : 1;
                    if (s.yav == 12 && !cfmt.bstemdown) {
                        if (!s.prev || s.prev.type == BAR) {
                            for (t = s.next; t; t = t.next) {
                                if (t.type == NOTE || t.type == BAR) break
                            }
                            if (t && t.type == NOTE && t.yav < 12) s.stem = 1
                        } else {
                            dy = s.yav - lasty;
                            if (dy > -7 && dy < 7) s.stem = laststem
                        }
                    }
                }
            } else {
    ...
    function same_head(s1, s2) {
        ...
        if ((l1 = s1.dur) >= BASE_LEN) return false;
        if ((l2 = s2.dur) >= BASE_LEN) return false;
        if (s1.stemless && s2.stemless) return false;
        if (s1.dots != s2.dots) {
            if (s1.shiftunison && s1.shiftunison & 1 || s1.dots * s2.dots != 0) return false
        }
        ...
        if (head == 0) head = voice_tb[s1.v].scale < voice_tb[s2.v].scale ? 2 : 1;
        if (head == 1) {
            s2.nohdi1 = i21;
            s2.nohdi2 = i22;
    ...
    function set_right(s, right) {
        ...
        for (i = 0; i < MAXPIT; i++) right[i] = -100;
    ...
    function set_overlap() {
        var s, s1, s2, i, i1, i2, m, sd, t, dp, d, d2, dr, dr2, dx, left1 = [],
            right1 = [],
            left2 = [],
            right2 = [],
            pl, pr;
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.type != NOTE || s.invisible) continue;
            ...
            for (;;) {
                ...
                if (s2.type == NOTE && !s2.invisible && s2.st == s.st) break
            }
            if (!s2) continue;
            s1 = s;
            if (cur_sy.voices[s1.v].range < cur_sy.voices[s2.v].range) s2.dot_low = true;
    ...
    function set_stems() {
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            if ((scale = voice_tb[s.v].scale) != 1) slen *= (scale + 1) * .5;
    ...
    function check_bar(s) {
        ...
        while (s.type == CLEF || s.type == KEY || s.type == METER) {
            if (s.type == METER && s.time > p_voice.sym.time) insert_meter |= 1;
            s = s.prev;
            if (!s) return
        }
        ...
        bar_type = s.bar_type;
        if (bar_type == ":") return;
        if (bar_type[bar_type.length - 1] != ":") return;
        p_voice.bar_start = clone(s);
        if (bar_type[0] != ":") {
            p_voice.bar_start.bar_type = bar_type;
            if (s.prev && s.prev.type == BAR) unlksym(s);
            else s.bar_type = "|";
            return
        }
        if (bar_type == "::") {
    ...
    function sym_staff_move(st, s, sy) {
        while (1) {
            ...
            s = s.ts_next;
            if (s == tsnext || s.new_sy) break
        }
    }

    function set_piece() {
        var s, p_voice, st, v, nst, nv, empty = [],
            sy = cur_sy;
        ...
        function set_empty(sy) {
            ...
            for (st = 0; st < n; st++) {
                ...
                if (empty_fl == 3) {
                    while (i <= st) sy.staves[i++].empty = false
                }
            }
        }
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            if (s.new_sy) {
                ...
                set_empty(sy);
                for (; st <= nst; st++) sy.staves[st].empty = true;
                sy = sy.next;
                nst = sy.nstaff;
                if (nst > nstaff) nstaff = nst;
                for (; st <= nst; st++) reset_staff(st)
            }
            if (s.st > nstaff) {
                unlksym(s);
                continue
            }
            if (!empty[s.st]) continue;
            switch (s.type) {
                case GRACE:
                    empty[s.st] = false;
                    break;
                ...
                case MREST:
                    if (cfmt.staffnonote > 1) {
                        ...
                        if (cfmt.staffnonote != 0 || s.type == NOTE) empty[s.st] = false
                    }
                    break
            }
        }
        tsnext = s;
        for (st = 0; st <= nstaff; st++) sy.staves[st].empty = empty[st];
        set_empty(sy);
        set_top_bot();
        sy = cur_sy;
        for (st = 0; st < nstaff; st++) {
            if (sy.staves[st].empty) sym_staff_move(st, tsfirst, sy)
        }
        if (sy.next) {
            for (s = tsfirst; s; s = s.ts_next) {
                if (s.nl) break;
                if (s.new_sy) {
                    ...
                    for (st = 0; st < sy.nstaff; st++) {
                        if (sy.staves[st].empty) sym_staff_move(st, s, sy)
                    }
                    if (!sy.next) break
                }
            }
        }
        if (empty[nstaff]) staff_tb[nstaff].topbar = 0;
        init_music_line();
        if (!empty[nstaff]) insert_meter &= ~1;
        if (!tsnext) return;
        s = tsnext;
        s.nl = false;
        s = s.ts_prev;
        ...
        for (v = 0; v < nv; v++) {
            ...
            if (!s) voice_tb[v].sym = null
        }
    }

    function set_sym_glue(width) {
        var space, beta0, alfa, beta;
        var some_grace, s = tsfirst,
            xmin = 0,
            x = 0,
            xmax = 0;
        while (1) {
            if (s.type == GRACE) some_grace = true;
            if (s.seqst) {
                space = s.space;
                xmin += s.shrink;
                if (space < s.shrink) space = s.shrink;
                x += space
            }
            if (!s.ts_next) break;
            s = s.ts_next
        ...
        if (tsnext) {
            ...
            if (x < width) {
                ...
                if (beta >= beta_last) {
                    ...
                    if (beta < width * (1 - cfmt.stretchlast)) width = beta
                }
            }
        }
        var spafac = width / x;
        ...
        while (1) {
            if (s.seqst) {
                space = s.shrink;
                if (s.space != 0) {
                    if (space < s.space * spafac) space = s.space * spafac;
        ...
        if (alfa != 0) {
            ...
            x = 0;
            for (; s; s = s.ts_next) {
                if (s.seqst) x = s.xmax * beta + s.x * (1 - beta);
                s.x = x
            }
        }
        if (some_grace) {
    ...
    function get_st_lines(param) {
        var n, val;
        if (param[0] == "|" || param[0] == ".") return param;
        n = parseInt(param);
        switch (n) {
            case 0:
                return "...";
    ...
    function do_pscom(text) {
        ...
        function set_v_param(k, v) {
            if (curvoice) {
                ...
                if (!parse.voice_param) parse.voice_param = {};
        ...
        switch (cmd) {
            ...
            case "repeat":
                ...
                if (curvoice.last_sym.type == BAR) s.repeat_n = n;
                else s.repeat_n = -n;
                sym_link(s);
                return;
            case "sep":
            ...
            case "setbarnb":
                ...
                if (parse.state >= 2) glovar.new_nbar = val;
                else cfmt.measurefirst = val;
                return;
            case "staff":
                if (parse.state != 3) {
                    if (parse.state != 2) return;
            ...
            case "staffbreak":
                ...
                if (param[0] >= "0" && param[0] <= "9") {
                    ...
                    if (param[param.length - 1] == "f") s.stbrk_forced = true
                } else {
                    s.xmx = .5 * CM;
            ...
            case "transpose":
                ...
                if (!curvoice) {
                    if (parse.state == 0) cfmt.transpose = val;
                ...
                if (!s) {
                    ...
                    if (curvoice.key.k_none) curvoice.key.k_sf = 0;
                    return
                }
                for (;;) {
                    if (s.type == KEY) break;
                ...
                if (curvoice.okey.k_none) s.k_none = curvoice.okey.k_none;
                if (curvoice.okey.k_a_acc) s.k_a_acc = curvoice.okey.k_a_acc;
            ...
            case "voicecolor":
                ...
                if (param != "#000000" && param != "black") s.color = param;
                sym_link(s);
                return;
            case "voicecombine":
                val = parseInt(param);
    ...
    function parse_kv_args(a) {
        ...
        if (!voice_param) voice_param = parse.voice_param = {};
        while (1) {
            item = a.shift();
            if (!item) break;
            switch (item) {
                ...
                default:
                    switch (item.slice(0, 4)) {
                        ...
                        case "perc":
                            ...
                            break;
                        default:
                            if ("GFC".indexOf(item[0]) >= 0) s = new_clef(item);
                            else if (item[item.length - 1] == "=") a.shift();
                            break
                    }
                    break
            }
    ...
    function new_key(param) {
        ...
        if (!key_end) {
            ...
            if (param.slice(0, 4) == "exp ") {
                ...
                if (!param) parse.line.error("no accidental after 'exp'");
                s.k_exp = true
            }
            c = param[0];
            if (c == "^" || c == "_" || c == "=") {
    ...
    function new_meter(text) {
        ...
        if (parse.state != 3) {
            ...
            if (parse.state >= 1) {
                if (!glovar.ulen) {
                    if (wmeasure >= BASE_LEN * 3 / 4 || wmeasure == 0) glovar.ulen = BASE_LEN / 8;
                    else glovar.ulen = BASE_LEN / 16
                }
                for (v = 0; v < voice_tb.length; v++) {
                    voice_tb[v].meter = s;
    ...
    function new_tempo(text) {
        ...
        if (cfmt.writefields.indexOf("Q") < 0) return;
        if (text[0] == '"') {
            i = text.indexOf('"', 1);
            if (i < 0) {
        ...
        if (curvoice.v != par_sy.top_voice) return;
        sym_link(s)
    }
    ...
    function do_info(info_type, text) {
        switch (info_type) {
            ...
            case "L":
                ...
                if (parse.state == 2) goto_tune();
                if (parse.state != 3) glovar.ulen = Number(text);
                else curvoice.ulen = Number(text);
                break;
            case "M":
                new_meter(text);
                break;
            ...
            case "P":
                ...
                if (parse.state == 2) goto_tune();
                if (cfmt.writefields.indexOf("P") < 0) break;
                var s = {
                    type: PART,
                    ctx: parse.ctx,
                    istart: parse.istart,
            ...
            case "V":
                ...
                if (parse.state == 2) goto_tune();
                get_voice(text);
                if (!curvoice.last_sym && parse.voice_opts) voice_filter();
                break;
            case "K":
    ...
    function adjust_dur(s) {
        ...
        if (!s2) return;
        if (s2.type == MREST || s2.type == BAR) return;
        while (s2.type != BAR && s2.prev) s2 = s2.prev;
        time = s2.time;
    ...
    function new_bar() {
        ...
        if (s.text && curvoice.norepbra && !curvoice.second) s.norepbra = true;
        ...
        if (s2 && s2.type == KEY) {
            ...
            if (!curvoice.last_sym) curvoice.sym = null;
            sym_link(s);
            s.next = s2;
            s2.prev = s;
        ...
        while (i < p.length) {
            switch (p[i]) {
                ...
                case "[":
                    ...
                    if (brace + bracket == 0) flags |= OPEN_BRACKET;
                    ...
                    for (; i < p.length; i++) {
                        switch (p[i]) {
                            ...
                            case "]":
                                ...
                                bracket--;
                                if (brace + bracket == 0) flags |= CLOSE_BRACKET;
                            ...
                            case "}":
                                ...
                                brace--;
                                if (bracket == 0) flags |= CLOSE_BRACE;
    ...
    function identify_note(s, dur) {
        var head, dots, flags;
        if (dur % 12 != 0) parse.line.error("Invalid note duration " + dur);
        dur /= 12;
        if (dur == 0) parse.line.error("Note too short");
    ...
    function parse_acc_pit(line) {
        ...
        if (acc && (c >= "1" && c <= "9") || c == "/") {
            ...
            micro_n = nd[0];
            micro_d = nd[1];
            if (micro_d == 1) micro_d = curvoice.microscale;
            else micro_d *= 2;
            c = line.char()
        }
    ...
    function parse_vpos() {
        var c, line = parse.line,
            ti1 = 0;
        if (line.buffer[line.index - 1] == "." && !a_dcn) ti1 = SL_DOTTED;
        c = line.next_char();
        if (c == "'") {
            ti1 += SL_ABOVE;
            line.index++
    ...
    function new_note(grace, tuplet_fact) {
        var note, s, in_chord, c, dcn, i, n, s2, nd, res, num, dur, sl1 = 0,
            line = parse.line,
            a_dcn_sav = a_dcn;
        a_dcn = undefined;
        ...
        switch (c) {
            ...
            case "Z":
                s.type = MREST;
                c = line.next_char();
                if (c > "0" && c <= "9") s.nmes = line.get_int();
                else s.nmes = 1;
            ...
            case "[":
                ...
                c = line.next_char();
            default:
                if (curvoice.microscale) s.microscale = curvoice.microscale;
                s.notes = [];
                while (1) {
                    if (in_chord) {
                        while (1) {
                    ...
                    if (!note) return null;
                    if (curvoice.octave) note.apit = note.pit += curvoice.octave * 7;
                    if (sl1) {
                        note.sl1 = sl1;
                        if (s.sl1) s.sl1++;
                        else s.sl1 = 1;
        ...
        if (s.notes) {
            if (s.type == NOTE && curvoice.transpose) note_transpose(s);
            if (!s.grace) {
                ...
                if (tuplet_fact) s.dur *= tuplet_fact;
                ...
                if (num) {
                    ...
                    if (num > 0) {
                        ...
                        for (i = 0; i <= s.nhd; i++) s.notes[i].dur = s.notes[i].dur * n / num;
                        s2.dur /= num;
                        s2.dur_orig /= num;
                        ...
                        for (i = 0; i <= s2.nhd; i++) s2.notes[i].dur = s2.notes[i].dur * n / num
                    }
            ...
            if (s.type == NOTE) {
                ...
                if (curvoice.map && maps[curvoice.map]) {
                    for (i = 0; i <= s.nhd; i++) set_map(s.notes[i])
                }
                if (s.nhd != 0) sort_pitch(s)
            } else {
                dur = s.dur_orig;
                if (dur == curvoice.wmeasure) {
                    if (dur < BASE_LEN * 2) dur = BASE_LEN;
                    else if (dur < BASE_LEN * 4) dur = BASE_LEN * 2;
                    else dur = BASE_LEN * 4
                }
    ...
    function parse_music_line() {
        var slur_start = 0,
            s, grace, sappo, dcn, i, c, idx, type, last_note_sav, a_dcn_sav, s_tuplet, s_tuplet_up, tuplet_fact = 1,
            tuplet_fact_up;
        var line = parse.line;
        ...
        while (1) {
            ...
            switch (type[0]) {
                case " ":
                    if (curvoice.last_note) curvoice.last_note.beam_end = true;
                    break;
                case "\n":
                    if (cfmt.barsperstaff) break;
                    if (par_sy.voices[curvoice.v].range == 0 && curvoice.last_sym) curvoice.last_sym.eoln = true;
                    break;
                case "&":
                    c = line.next_char();
                    if (c == ")") {
                ...
                case "(":
                    ...
                    if (c > "0" && c <= "9") {
                        ...
                        if (c == ":") {
                            ...
                            if (qplet == 0) qplet = curvoice.wmeasure % 9 == 0 ? 3 : 2
                        }
                        s = {
                            type: TUPLET,
                            tuplet_p: pplet,
                ...
                case "-":
                    ...
                    for (i = 0; i <= s.nhd; i++) {
                        if (s.notes[i].ti1 == 0) s.notes[i].ti1 = tie_pos;
                        else if (s.nhd == 0) line.error("Too many ties")
                    }
                    s.ti1 = true;
                    continue;
                case "[":
                ...
                case "n":
                    ...
                    if (grace) {
                        if (s_tuplet) s.in_tuplet = true
                    } else if (s_tuplet && s.notes) {
                        s.in_tuplet = true;
                        s_tuplet.tuplet_n--;
                        if (s_tuplet_up) s_tuplet_up.tuplet_n--;
                ...
                case "}":
                    ...
                    break;
                case "\\":
                    if (line.index == line.buffer.length - 1) return;
        ...
        if (cfmt.breakoneoln && curvoice.last_note) curvoice.last_note.beam_end = true;
        if (cfmt.barsperstaff) return;
        if (char_tb["\n".charCodeAt(0)] == "\n" && par_sy.voices[curvoice.v].range == 0 && curvoice.last_sym) curvoice.last_sym.eoln = true
    }
    ...
    function out_str(str) {
        ...
        for (i = 0; i < str.length; i++) {
            ...
            switch (c) {
                ...
                case "&":
                    if (str[i + 1] == "#") break;
                    if (str[i + 2] == "t" && str[i + 3] == ";" && (str[i + 1] == "l" || str[i + 1] == "g")) break;
                    if (str[i + 1] == "a" && str[i + 2] == "p" && str[i + 3] == "o" && str[i + 4] == "s" && str[i + 5] == ";") break;
                    if (str[i + 1] == "q" && str[i + 2] == "u" && str[i + 3] == "o" && str[i + 4] == "t" && str[i + 5] == ";") break;
                    if (str[i + 1] == "a" && str[i + 2] == "m" && str[i + 3] == "p" && str[i + 4] == ";") break;
                    c = "&amp;";
                    break;
                case "$":
                    c = str[++i];
                    if (c == "0") {
                        if (gene.curfont == gene.deffont) break;
                        gene.curfont = gene.deffont;
                        c = "</tspan>"
                    } else if (c >= "1" && c <= "9") {
                        new_font = get_font("u" + c);
                        if (!new_font) break;
                        if (new_font == gene.curfont) break;
                        if (gene.curfont != gene.deffont) output.push("</tspan>");
    ...
    function xy_str(x, y, str, action, dx) {
        ...
        out_sxsy(x, '" y="', y);
        if (action == "c") output.push('" text-anchor="middle">');
        else if (action == "j") output.push('" textLength="' + dx.toFixed(2) + '">');
        else if (action == "r") output.push('" text-anchor="end">');
        else output.push('">');
        out_str(str);
        output.push("</text>\n")
    }
    ...
    function trim_title(title, is_subtitle) {
        ...
        if (cfmt.titletrim) {
            i = title.lastIndexOf(", ");
            if (i < 0 || title[i + 2] < "A" || title[i + 2] > "Z" || i < title.length - 7 || title.indexOf(" ", i + 3) >= 0) i = 0
        }
    ...
    function write_title(title, is_subtitle) {
        ...
        if (cfmt.titleleft) xy_str(0, sz * .2, title);
        else xy_str((cfmt.pagewidth - cfmt.leftmargin - cfmt.rightmargin) / 2 / cfmt.scale, sz * .2, title, "c")
    }
    ...
    function write_text(text, action) {
        if (action == "s") return;
        set_font("text");
        var strlw = (cfmt.pagewidth - cfmt.leftmargin - cfmt.rightmargin) / cfmt.scale,
    ...
    function put_words(words) {
        ...
        for (i = 0; i < i_end || i2 < nw; i++) {
            ...
            if (i < i_end && words[i].length == 0) blk_out();
    ...
    function write_headform(lwidth) {
        ...
        while (1) {
            ...
            if (c < "A" || c > "Z") {
                if (c == "+") {
                    if (fmt.lenth == 0 || fmt[fmt.length - 1] == "+") continue;
                    fmt = fmt.slice(0, -1) + "+"
                } else if (c == ",") {
                    if (fmt[fmt.length - 1] == "+") fmt = fmt.slice(0, -1) + "l";
                    fmt += "\n"
                }
                continue
            }
    ...
    function write_heading() {
        ...
        if (parse.ckey.k_bagpipe && !cfmt.infoline && cfmt.writefields.indexOf("R") >= 0) rhythm = info.R;
        if (rhythm) {
    ...
    function set_g() {
        ...
        if (stv_g.scale != 1) {
            if (stv_g.st >= 0) {
                if (staff_tb[stv_g.st].y == 0) output.push("!S" + stv_g.st + "!");
                else output.push(staff_tb[stv_g.st].scale_str)
            } else {
                if (staff_tb[0].y == 0) output.push("!V" + stv_g.v + "!");
                else output.push(voice_tb[stv_g.v].scale_str)
            }
        }
        if (stv_g.color) {
            if (stv_g.scale != 1) output.push(" ");
    ...
    function delayed_update() {
        ...
        for (st = 0; st <= nstaff; st++) {
            if (staff_tb[st].output.length == 0) continue;
            output.push('<g transform="translate(0,' + (-staff_tb[st].y).toFixed(2));
            if (staff_tb[st].staffscale != 1) output.push(") scale(" + staff_tb[st].staffscale.toFixed(2));
            output.push(')">\n');
    ...
    function out_sxsy(x, sep, y) {
        if (!stv_g.trans) {
            ...
            if (stv_g.scale != 1) {
                if (stv_g.st < 0) y = (posy - y) / stv_g.scale;
                else y = stv_g.dy - y
            } else {
                y = posy - y
            }
    ...
    function xypath(x, y, fill) {
        if (fill) output.push('<path class="fill" d="m');
        else output.push('<path class="stroke" d="m');
        out_sxsy(x, " ", y);
        output.push("\n")
    }
    ...
    function out_bnum(x, y, str, erase) {
        ...
        if (erase) {
            var i, w = .6;
            for (i = 0; i < str.length; i++) w += cwid(str[i]);
            w *= 12;
    ...
    function out_stem(x, y, h, grace, nflags, straight) {
        ...
        output.push('<path class="stroke" d="m');
        out_sxsy(x, " ", y);
        if (stv_g.st < 0) slen /= stv_g.scale;
    ...
    function out_cresc(x, y, val, defl) {
        ...
        if (defl.nost) output.push(" -2.2m0 -3.6l" + val.toFixed(2) + ' -2.2"/>\n');
    ...
    function out_dim(x, y, val, defl) {
        ...
        out_sxsy(x, " ", y + 5);
        output.push("l" + val.toFixed(2));
        if (defl.noen) output.push(" -2.2m0 -3.6l" + (-val).toFixed(2) + ' -2.2"/>\n');
        else output.push(" -4l" + (-val).toFixed(2) + ' -4"/>\n')
    }
    ...
    function out_deco_val(x, y, name, val, defl) {
        ...
        else error(1, null, "No function for decoration '" + name + "'")
    }
    ...
    function svg_flush() {
        if (multicol || !output || !user.img_out) return;
        var img_title;
        if (info.X) {
            img_title = info.X + ".";
            if (info.T) img_title += " " + info.T.split("\n")[0];
        ...
        if (cfmt.scale != 1) user.img_out('<g transform="scale(' + cfmt.scale.toFixed(2) + ')">');
        if (svgobj) {
            svgobj.setg(0);
            output.push(svgbuf);
            svgbuf = ""
        ...
        output = [];
        if (cfmt.scale != 1) user.img_out("</g>");
        user.img_out("</svg>");
        font_style = "";
        if (cfmt.fullsvg) {
            defined_glyph = {};
    ...
    function voice_filter() {
        ...
        for (opt in parse.voice_opts) {
            ...
            if (sel.test(curvoice.id) || sel.test(curvoice.nm)) {
                for (i = 0; i < parse.voice_opts[opt].length; i++) do_pscom(parse.voice_opts[opt][i])
            }
        }
    }
    ...
    function mrest_expand(s) {
        ...
        while (--nb > 0) {
            ...
            s2 = sym_add(p_voice);
            s2.type = REST;
            if (s.invisible) s2.invisible = true;
            s2.dur = dur;
            s2.head = "full";
            s2.nflags = -2;
            p_voice.time += dur
    ...
    function sort_all() {
        var s, s2, p_voice, v, time, w, wmin, ir, multi, prev, new_sy, fl, nb, vtb = [],
            vn = [],
            mrest_time = -1;
        for (v = 0; v < voice_tb.length; v++) vtb[v] = voice_tb[v].sym;
        var sy = cur_sy,
            set_sy = true;
        while (1) {
            if (set_sy) {
            ...
            if (time == mrest_time) {
                ...
                for (ir = 0; ir < vn.length; ir++) {
                    ...
                    if (v == undefined) break;
                    s = vtb[v];
                    if (!s || s.time != time || weight_tb[s.type] != wmin) continue;
                    if (s.type != MREST) {
                        mrest_time = -1;
                        break
                    }
                ...
                if (mrest_time < 0) {
                    for (ir = 0; ir < vn.length; ir++) {
                        ...
                        if (v == undefined) break;
                        s = vtb[v];
                        if (s && s.type == MREST) mrest_expand(s)
                    }
                }
            }
            for (ir = 0; ir < vn.length; ir++) {
    ...
    function compr_voice() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            p_voice = voice_tb[v];
            if (p_voice.ignore) p_voice.ignore = false;
            for (s = p_voice.sym; s; s = s.next) {
    ...
    function new_system() {
        ...
        if (!par_sy) {
            ...
            for (v = 0; v < voice_tb.length; v++) {
                ...
                if (p_voice.stafflines != undefined) sy_staff.stafflines = p_voice.stafflines;
    ...
    function go_global_time(s, symsel) {
        ...
        if (symsel.bar <= 1) {
            if (symsel.bar == 1) {
                for (s2 = s; s2; s2 = s2.ts_next) {
                    if (s2.type == BAR && s2.time != 0) break
                }
                if (s2.time < voice_tb[cur_sy.top_voice].meter.wmeasure) s = s2
            }
        } else {
            for (; s; s = s.ts_next) {
                if (s.type == BAR && s.bar_num >= symsel.bar) break
            }
            if (!s) return null;
            if (symsel.seq != 0) {
                var seq = symsel.seq;
    ...
    function do_clip() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            if (!s2) p_voice.sym = null
        }
        delete s.ts_prev.ts_next
    }

    function set_bar_num() {
        var s, s2, s3, tim, v = cur_sy.top_voice,
            wmeasure = voice_tb[v].meter.wmeasure,
            bar_rep = gene.nbar;
        if (wmeasure == 0) wmeasure = 1;
        ...
        for (; s; s = s.ts_next) {
            switch (s.type) {
                ...
                case MREST:
                    bar_num += s.nmes - 1;
                    while (s.ts_next && s.ts_next.type != BAR) s = s.ts_next;
                    break;
                case BAR:
                    if (s.bar_num) {
                        bar_num = s.bar_num;
                    ...
                    do {
                        if (s2.type == BAR && s2.text && !cfmt.contbarnb) {
                            if (s2.text[0] == "1") bar_rep = bar_num;
                            else bar_num = bar_rep;
                            break
                        }
                        s2 = s2.next
    ...
    function key_transpose(s_key, transpose) {
        var sf, t;
        if (transpose > 0) t = Math.floor(transpose / 3);
        else t = -Math.floor(-transpose / 3);
    ...
    function note_transpose(s) {
        var i, j, n, d, a, i1, i3, i4, note, m = s.nhd,
        ...
        for (i = 0; i <= m; i++) {
            ...
            if (curvoice.ckey.k_none) {
                ...
                for (j = 0; j < curvoice.ckey.a_acc.length; j++) {
                    if ((i4 + 16 * 7 - curvoice.ckey.a_acc[j].pits) % 7 == 0) break
                }
                if (j < curvoice.ckey.a_acc.length) continue
            } else {
                continue
            }
            i1 = note.acc;
    ...
    function get_staves(cmd, param) {
        ...
        if (no_sym || maxtime == 0 && staves_found < 0) {
            for (v = 0; v < par_sy.voices.length; v++) par_sy.voices[v].range = -1
        ...
        if (cmd[1] == "t") {
            for (i = 0; i < a_flags.length; i++) {
                ...
                if ((flags & (OPEN_BRACE | CLOSE_BRACE)) == (OPEN_BRACE | CLOSE_BRACE) || (flags & (OPEN_BRACE2 | CLOSE_BRACE2)) == (OPEN_BRACE2 | CLOSE_BRACE2)) continue;
                if (a_flags[i + 1].flags != 0) continue;
                if (flags & OPEN_PARENTH || a_flags[i + 2].flags & OPEN_PARENTH) continue;
        ...
        for (i = 0; i < a_flags.length; i++) {
            ...
            if (flags & OPEN_PARENTH) {
                ...
                while (i < a_flags.length - 1) {
                    ...
                    p_voice.st = p_voice.cst = par_sy.voices[v].st = st;
                    if (p_flags.flags & CLOSE_PARENTH) break
                }
        ...
        for (v = 0; v < voice_tb.length; v++) {
            if (par_sy.voices[v].range < 0) continue;
            p_voice = voice_tb[v];
    ...
    function get_vover(type) {
        ...
        if (type == "|" || type == ")") {
            ...
            if (curvoice.time != vover.mxtime) line.error("Wrong duration in voice overlay");
        ...
        if (!p_voice2) {
            ...
            for (v = 0; v < par_sy.voices.length; v++) {
                if (par_sy.voices[v].range > range) par_sy.voices[v].range += f_clone + 1
            }
        ...
        if (curvoice.microscale) p_voice2.microscale = curvoice.microscale;
        if (!vover) {
            vover = {
                bar: true,
            ...
            for (s = curvoice.last_sym;; s = s.prev) {
                if (s.type == BAR || s.time <= time) break
            }
            vover.time = s.time
        } else {
            if (!vover.p_voice) {
            ...
            } else if (curvoice.time != vover.mxtime) line.error("Wrong duration in voice overlay")
        }
    ...
    function set_voice_param(p_voice) {
        var k;
        if (cfmt.microscale) p_voice.microscale = cfmt.microscale;
        if (glovar.stafflines != undefined && p_voice.stafflines == undefined) p_voice.stafflines = glovar.stafflines;
        if (glovar.staffscale) p_voice.staffscale = glovar.staffscale;
        if (cfmt.transpose && !p_voice.transpose) p_voice.transpose = cfmt.transpose;
        for (k in parse.voice_param) p_voice[k] = parse.voice_param[k];
        if (!p_voice.wmeasure) p_voice.wmeasure = p_voice.meter.wmeasure
    }

    function get_key_clef(kc) {
        var v, p_voice, s2, s_key = kc[0],
            o_key = clone(s_key),
            s_clef = kc[1];
        if (s_key.k_sf) {
            if (!s_key.k_exp && s_key.k_a_acc) set_k_acc(s_key)
        }
        if (parse.state == 1) {
            if (s_key.k_sf == undefined && !s_key.k_a_acc) s_key.k_sf = 0;
            if (cfmt.transpose) key_transpose(s_key, cfmt.transpose);
            for (v = 0; v < voice_tb.length; v++) {
                p_voice = voice_tb[v];
                p_voice.ckey = s_key;
                ...
                if (s_key.k_none) p_voice.key.k_sf = 0
            }
            parse.okey = o_key;
            parse.ckey = s_key;
            if (s_clef) {
        ...
        if (curvoice.transpose) key_transpose(s_key, curvoice.transpose);
        if (s_clef) get_clef(s_clef);
        if (is_voice_sig()) {
        ...
        if (s_key.k_sf == undefined && !s_key.k_a_acc) return;
    ...
    function new_voice(id) {
        ...
        if (parse.state == 3) set_voice_param(p_voice);
        voice_tb.push(p_voice);
        par_sy.voices[v] = {
            range: -1
        };
    ...
    function goto_tune() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            if (p_voice.key.k_bagpipe && p_voice.posit.std == 0) p_voice.posit.std = SL_BELOW;
            if (p_voice.key.k_none) p_voice.key.k_sf = 0;
    ...
    function get_lyrics(text, cont) {
        var s, word, p, i, ly;
        if (curvoice.ignore) return;
        if (curvoice.posit.voc != SL_HIDDEN) curvoice.have_ly = true;
        if (cont) {
        ...
        while (1) {
            ...
            while (s && (s.type != NOTE || s.grace)) s = s.next;
            if (!s) {
    ...
    function ly_width(s, wlw) {
        ...
        for (i = 0; i < a_ly.length; i++) {
            ...
            for (k = s.next; k; k = k.next) {
                switch (k.type) {
                    case NOTE:
                    case REST:
                        if (!k.a_ly || !k.a_ly[i] || k.a_ly[i].w == 0) xx -= 9;
                        else if (k.a_ly[i].t == "-\n" || k.a_ly[i].t == "_\n") xx -= shift;
                        else break;
                        if (xx <= 0) break;
                        continue;
        ...
        if (align > 0) {
            for (i = 0; i < a_ly.length; i++) {
                ly = a_ly[i];
                if (ly.t[0] >= "0" && ly.t[0] <= "9") ly.shift = align
            }
        }
        return wlw
    }
    ...
    function draw_lyrics(p_voice, nly, y, incr) {
        ...
        if (incr > 0) {
            ...
            j = nly - 1;
            nly = -1;
            if (y < top + cfmt.vocalspace - font.size) y = top + cfmt.vocalspace - font.size
        }
        desc = font.size * .25;
        for (; j != nly; j += incr) {
            Y = y + desc;
            ...
            for (s = p_voice.sym;; s = s.next)
                if (s.type != CLEF && s.type != KEY && s.type != METER) break;
            if (s.prev) lastx = s.prev.x;
            else tsfirst.x;
            x0 = 0;
            lskip = font.size * 1.1;
            for (; s; s = s.next) {
                ...
                if (ly.font != font) {
                    ...
                    if (lskip < font.size * 1.1) lskip = font.size * 1.1
                }
                p = ly.t;
                w = ly.w;
                shift = ly.shift;
    ...
    function draw_all_lyrics() {
        var p_voice, s, v, nly, i, x, y, w, lyst_tb = new Array(nstaff),
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            if (p_voice.have_ly) {
                for (s = p_voice.sym; s; s = s.next) {
                    ...
                    if (bot > y) bot = y;
                    if (nly < a_ly.length) nly = a_ly.length
                }
            } else {
            ...
            nly_tb[v] = nly;
            if (nly == 0) continue;
            if (p_voice.posit.voc != 0) above_tb[v] = p_voice.posit.voc == SL_ABOVE;
            else if (voice_tb[p_voice.v + 1] && voice_tb[p_voice.v + 1].st == st && voice_tb[p_voice.v + 1].have_ly) above_tb[v] = true;
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            st = p_voice.st;
            set_dscale(st);
            if (nly_tb[v] > 0) lyst_tb[st].bot = draw_lyrics(p_voice, nly_tb[v], lyst_tb[st].bot, 1)
        }
        while (--i >= 0) {
            v = rv_tb[i];
            p_voice = voice_tb[v];
    ...
    function parse_gchord(type) {
        ...
        if (!gchord) gchord = cnv_escape(text);
        else gchord += "\n" + cnv_escape(text)
    }
    ...
    function gch_transpose(s) {
        ...
        i3 = i1 + i2 + a * 7;
        i4 = cgd2cde[(i3 + 16 * 7) % 7];
        if (!latin) new_txt = note_names[i4];
        else new_txt = latin_names[i4];
        i1 = (Math.floor((i3 + 1 + 21) / 7) + 2 - 3 + 32 * 5) % 5;
        new_txt += acc_name[i1];
        ip2 = p.indexOf("/", ip);
        if (ip2 < 0) {
    ...
    function gch_build(s) {
        if (curvoice.posit.gch == SL_HIDDEN) return;
        var src, gch, w, xspc, x_abs, y_abs, ix, gch_place = curvoice.posit.gch == SL_BELOW ? -1 : 1,
        ...
        if (curvoice.transpose) gch_transpose(s);
        const GCHPRE = .4;
        for (ix = 0; ix < s.a_gch.length; ix++) {
            gch = s.a_gch[ix];
            if (gch.type == "g") {
            ...
            if (gch.type == "@" && !user.anno_start) continue;
    ...
    function draw_gchord(s, gchy_min, gchy_max) {
        ...
        for (ix = 0; ix < n; ix++) {
            ...
            switch (gch.type) {
                ...
                case "@":
                    y = gch.y + yav;
                    if (y > 0) y_set(s.st, 1, x, 1, y + h * .8 + 2);
                    else y_set(s.st, 0, x, 1, y - h * .2 - 4);
                    break
            }
