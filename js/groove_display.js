...
if (typeof GrooveDisplay === 'undefined') {
  ...
  (function () {
    ...
    root.loadjscssfile = function (filename, filetype) {
      ...
      if (root.filesadded.indexOf('[' + filename + ']') != -1) return; // file already added
    ...
    root.GrooveDBFormatPutGrooveInHTMLElement = function (HtmlTagId, GrooveDBTabIn) {
      ...
      var combinedSnareTab = myGrooveUtils.mergeDrumTabLines(
        GrooveDBTabIn.snareAccentTab,
        GrooveDBTabIn.snareOtherTab
      );
      var combinedKickTab = myGrooveUtils.mergeDrumTabLines(
        GrooveDBTabIn.kickTab,
        GrooveDBTabIn.footOtherTab
      );

      if (GrooveDBTabIn.div !== undefined && !isNaN(GrooveDBTabIn.div))
        myGrooveData.timeDivision = GrooveDBTabIn.div;
      if (GrooveDBTabIn.tempo !== undefined && !isNaN(GrooveDBTabIn.tempo))
        myGrooveData.tempo = GrooveDBTabIn.tempo;
      if (GrooveDBTabIn.swingPercent !== undefined && !isNaN(GrooveDBTabIn.swingPercent))
        myGrooveData.swingPercent = GrooveDBTabIn.swingPercent;
      if (GrooveDBTabIn.measures !== undefined && !isNaN(GrooveDBTabIn.measures))
        myGrooveData.numberOfMeasures = GrooveDBTabIn.measures;
      if (
        GrooveDBTabIn.notesPerTabMeasure !== undefined &&
        !isNaN(GrooveDBTabIn.notesPerTabMeasure)
      )
        myGrooveData.notesPerMeasure = GrooveDBTabIn.notesPerTabMeasure;
      if (GrooveDBTabIn.stickingTab !== undefined)
        myGrooveData.sticking_array = myGrooveUtils.noteArraysFromURLData(
          'Stickings',
          GrooveDBTabIn.stickingTab,
          GrooveDBTabIn.notesPerTabMeasure,
          GrooveDBTabIn.measures
        );
      if (GrooveDBTabIn.hihatTab !== undefined)
        myGrooveData.hh_array = myGrooveUtils.noteArraysFromURLData(
          'H',
          GrooveDBTabIn.hihatTab,
          GrooveDBTabIn.notesPerTabMeasure,
          GrooveDBTabIn.measures
        );
      myGrooveData.snare_array = myGrooveUtils.noteArraysFromURLData(
        'S',
        combinedSnareTab,
        GrooveDBTabIn.notesPerTabMeasure,
        GrooveDBTabIn.measures
      );
      myGrooveData.kick_array = myGrooveUtils.noteArraysFromURLData(
        'K',
        combinedKickTab,
        GrooveDBTabIn.notesPerTabMeasure,
        GrooveDBTabIn.measures
      );
      if (GrooveDBTabIn.tom1Tab !== undefined)
      ...
      if (GrooveDBTabIn.tom4Tab !== undefined)
        myGrooveData.toms_array[3] = myGrooveUtils.noteArraysFromURLData(
          'T4',
          GrooveDBTabIn.tom4Tab,
          GrooveDBTabIn.notesPerTabMeasure,
          GrooveDBTabIn.measures
        );
      ...
      document.getElementById(HtmlTagId).innerHTML =
        '' +
        '<div class="Printable"><div id="' +
        svgTargetId +
        '" class="svgTarget"  style="display:inline-block"></div></div>\n' +
        '<div class="nonPrintable"><div id="' +
        midiPlayerTargetId +
        '" ></div></div>\n';
    ...
    root.AddGrooveDisplayToElementId = function (
      HtmlTagId,
      GrooveDefinition,
      showPlayer,
      linkToEditor,
    ...
    root.AddGrooveDisplayToPage = function (
      ...
      window.addEventListener(
        'load',
        function () {
          root.AddGrooveDisplayToElementId(
            HTMLElementID,
            URLEncodedGrooveData,
            showPlayer,
            linkToEditor,
