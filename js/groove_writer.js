...
function GrooveWriter() { "use strict";
	...
	function createMidiUrlFromClickableUI(MIDI_type) {
		...
		case "kick_16ths":
			...
			for (i = 0; i < numSections; i++) {
				...
				if (shouldDisplayPermutationForSection(i)) {
					...
					root.myGrooveUtils.MIDI_from_HH_Snare_Kick_Arrays(midiTrack, HH_Array, Snare_Array, new_kick_array, null, MIDI_type, metronomeFrequency, num_notes, num_notes_for_swing, swing_percentage, 4, 4);
				}
			}
			break;
		...
		case "snare_16ths": // use the hh & snare from the user
			...
			for (i = 0; i < numSections; i++) {
				if (shouldDisplayPermutationForSection(i)) {
					...
					root.myGrooveUtils.MIDI_from_HH_Snare_Kick_Arrays(midiTrack, HH_Array, new_snare_array, Kick_Array, null, MIDI_type, metronomeFrequency, num_notes, num_notes_for_swing, swing_percentage, 4, 4);
				}
			}
			break;
		...
		case "none":
			/* falls through */
		default:
			root.myGrooveUtils.MIDI_from_HH_Snare_Kick_Arrays(midiTrack, HH_Array, Snare_Array, Kick_Array, null, MIDI_type, metronomeFrequency, num_notes, class_notes_per_measure, swing_percentage, 4, 4);
		...
		for (i = indexStartForNotes; i < class_notes_per_measure + indexStartForNotes; i++) {
			...
			newHTML += ('\
														<div id="hi-hat' + i + '" class="hi-hat" onClick="myGrooveWriter.noteLeftClick(event, \'hh\', ' + i + ')" oncontextmenu="event.preventDefault(); myGrooveWriter.noteRightClick(event, \'hh\', ' + i + ')" onmouseenter="myGrooveWriter.noteOnMouseEnter(event, \'hh\', ' + i + ')">\
															...
															<div class="hh_ride note_part"   id="hh_ride' + i + '"><i class="fa fa-circle-o-notch fa-spin"></i></div>\
		...
		for (i = indexStartForNotes; i < class_notes_per_measure + indexStartForNotes; i++) {
			newHTML += ('' +
						...
						'<div class="snare_flam note_part" id="snare_flam' + i + '"><i class="fa ">' +
							...
							'	</style>' +
							'	<defs>' +
							'		<path id="flam_ghd" class="fill" d="m1.7-1c-1-1.7-4.5 0.2-3.4 2 1 1.7 4.5-0.2 3.4-2"></path>' +
							'		<ellipse id="flam_hd" rx="4.1" ry="2.9" transform="rotate(-20)" class="fill"></ellipse>' +
							'	</defs>' +
							...
							'		<path class="stroke" d="m50.5 58.34c2.9 3 11.6 3 14.5 0M69.5 53.34v-21"></path><use x="66.00" y="53.34" xlink:href="#flam_hd"></use>' +
							'	</g>' +
							'</svg>' + 
						'</i></div>' +
